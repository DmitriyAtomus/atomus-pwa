...
# ============ ЭТАП 22: ХРАНЕНИЕ ФАЙЛОВ (фото замечаний) ============
# Persistent volume на Railway — тот же каталог, где БД. Тогда фото переживают рестарт.
UPLOADS_DIR = os.path.join(os.path.dirname(DB_PATH), "uploads")
DEFECT_PHOTOS_DIR = os.path.join(UPLOADS_DIR, "defects")
os.makedirs(DEFECT_PHOTOS_DIR, exist_ok=True)
MAX_PHOTO_SIZE = 8 * 1024 * 1024   # 8 МБ на одно фото
MAX_PHOTOS_PER_DEFECT = 5
...
async def auth_telegram(request: web.Request) -> web.Response:
    ...
    return web.json_response({
        ...
        "user": {
            ...
            "position": position,
            "roles": sorted(roles),
            "permissions": sorted(await db.get_effective_permissions(chat_id, roles)),
        },
    })
...
# Простой in-memory rate-limit по IP для /api/auth/password.
# При перезапуске процесса сбрасывается — это OK, фронт всё равно тротлится.
_PASSWORD_LOGIN_ATTEMPTS: dict[str, list[float]] = {}
_PASSWORD_LOGIN_MAX = 10        # попыток
_PASSWORD_LOGIN_WINDOW = 600    # секунд = 10 минут


def _password_login_ratelimit_check(ip: str) -> bool:
    """True если можно делать ещё попытку, False если превышено. Также чистит старые."""
    now = time.time()
    bucket = _PASSWORD_LOGIN_ATTEMPTS.get(ip, [])
    # Чистка
    bucket = [t for t in bucket if now - t < _PASSWORD_LOGIN_WINDOW]
    _PASSWORD_LOGIN_ATTEMPTS[ip] = bucket
    return len(bucket) < _PASSWORD_LOGIN_MAX


def _password_login_record_attempt(ip: str) -> None:
    _PASSWORD_LOGIN_ATTEMPTS.setdefault(ip, []).append(time.time())


async def auth_password(request: web.Request) -> web.Response:
    """Вход по паролю — для сотрудников без Telegram.

    Тело запроса (POST):
        { "password": "..." }

    Бэк: ищет сотрудника по совпадению пароля (PBKDF2-перебор активных).
    Возвращает токен + user info, как auth_code.

    Защита:
    - Rate-limit по IP: 10 попыток / 10 минут
    - Минимум 6 символов
    - Timing-attack mitigation: всегда 200-500ms suspense на неуспех
    """
    # Rate-limit по IP
    ip = request.headers.get("X-Forwarded-For", "").split(",")[0].strip() \
        or request.remote or "unknown"
    if not _password_login_ratelimit_check(ip):
        return web.json_response(
            {"error": "rate_limit",
             "message": "Слишком много попыток. Подожди 10 минут."},
            status=429,
        )
    ...
    try:
        ...
        _password_login_record_attempt(ip)
        return web.json_response(
            {"error": "bad_request", "message": "Тело должно быть JSON"},
            status=400,
        )
    ...
    if not password:
        return web.json_response(
            {"error": "bad_password", "message": "Введите пароль"},
            status=400,
        )
    if len(password) < 6:
        # Без записи попытки в счётчик — это не серверная нагрузка
        return web.json_response(
            {"error": "bad_password", "message": "Пароль должен быть не короче 6 символов"},
            status=400,
        )

    # Перебор: для нескольких десятков сотрудников ~1-2 секунды,
    # параллельно работает как естественный throttle.
    emp = await db.verify_password_and_get_employee(password)

    if not emp:
        _password_login_record_attempt(ip)
        # Небольшая задержка против side-channel атак
    ...
    # Выбираем chat_id для токена:
    # 1) если у сотрудника есть реальный telegram_id — используем его (даёт совместимость
    #    со всем существующим кодом и доступ к бот-нотификациям)
    # 2) иначе — auth_chat_id (виртуальный отрицательный, проставленный
    #    в set_employee_password)
    chat_id = emp.get("telegram_id") or emp.get("auth_chat_id")
    if not chat_id:
        # Аномалия: пароль есть, но auth_chat_id не сгенерировался. Чиним на лету.
        chat_id = await _ensure_auth_chat_id(emp["id"])
        if not chat_id:
            return web.json_response(
                {"error": "internal", "message": "Не удалось создать сессию"},
                status=500,
            )

    chat_id = int(chat_id)

    # Проверяем что роли есть (теоретически могли быть удалены отдельной операцией)
    roles = get_user_roles(chat_id)
    if not roles:
        return web.json_response(
            {"error": "no_access",
    ...
    await db.log_action(chat_id, "login_password", "employee", emp["id"], name)
...
async def _ensure_auth_chat_id(emp_id: int) -> int | None:
    """Резервная функция: если у сотрудника по какой-то причине нет auth_chat_id,
    сгенерируем его и обновим запись. Возвращает chat_id или None при ошибке."""
    emp = await db.get_employee_by_id(emp_id)
    if not emp:
        return None
    aid = emp.get("auth_chat_id")
    if aid:
        return int(aid)
    new_aid = db._virtual_auth_chat_id(emp_id)
    import aiosqlite
    async with aiosqlite.connect(db.DB_PATH) as conn:
        await conn.execute(
            "UPDATE employees SET auth_chat_id = ?, updated_at = datetime('now') WHERE id = ?",
            (new_aid, emp_id),
        )
        await conn.commit()
    # Обновим кэш ролей
...
async def get_me(request: web.Request) -> web.Response:
    ...
    # Сначала пробуем найти по telegram_id (классика), потом по auth_chat_id (Этап 28)
    emp = await db.get_employee_by_telegram_id(chat_id)
    if not emp:
        emp = await db.get_employee_by_auth_chat_id(chat_id)

    # Этап 29: загружаем permissions и инфу о уровне доступа
    perms = await db.get_permissions_by_chat_id(chat_id)
    # v2.9.1: владелец чата / директор без access_level → даём все права
    if "director" in roles and not perms:
        perms = set(db.ALL_PERMISSION_KEYS)
    access_level = None
    if emp and emp.get("access_level_id"):
        access_level = await db.get_access_level_by_id(emp["access_level_id"])
    ...
    if emp:
        info = {
            ...
            "access_level_id": emp.get("access_level_id"),
            "access_level_name": access_level["name"] if access_level else None,
        }
    else:
        info = {
            "chat_id": chat_id,
...
async def get_employees(request: web.Request) -> web.Response:
    ...
    # ЭТАП 29: соберём карту level_id → level info (один SQL вместо N)
    levels = await db.get_access_levels(include_inactive=True)
    ...
    for e in employees:
        ...
        legacy_roles = sorted({r.strip() for r in roles_str.split(",") if r.strip()})
        # Effective roles от уровня доступа (если он есть)
        level_id = e.get("access_level_id")
        level_info = level_map.get(level_id) if level_id else None
        if level_info:
            perms = set(level_info["permissions"])
            effective_roles = sorted(db.derive_legacy_roles(perms))
        else:
            effective_roles = legacy_roles
        result.append({
            "id": e["id"],
            ...
            "access_level_name": level_info["name"] if level_info else None,
        })
...
async def get_employees_active(request: web.Request) -> web.Response:
    ...
    но без чувствительных полей). Возвращает только id, short_name, full_name, position, роли.
    """
...
async def create_assembly(request: web.Request) -> web.Response:
    ...
    # --- Извлечение полей ---
    # ЭТАП 23: тип работы
    work_type = (data.get("work_type") or "assembly").strip()
    if work_type not in db.WORK_TYPE_LABELS:
        return web.json_response(
            {"error": "bad_work_type",
             "message": "Некорректный тип работы"},
            status=400,
        )
    is_assembly = (work_type == "assembly")

    # model_id обязателен только для сборок
    ...
    if is_assembly:
        ...
        # Для не-сборок — модель опциональна
        try:
    ...
    # --- Проверка модели (только для сборок) ---
    if is_assembly:
        ...
        # Для не-сборок: модель опциональна, exec/IP не нужны
        model = None
        execution = None
        ip_class = None

    # IP-класс (только для сборок)
    if is_assembly and model:
    ...
    # --- Создание ---
    try:
        # ЭТАП 23: дополнительные поля для не-сборок
        description = str(data.get("description") or "").strip()[:1000] or None
        location    = str(data.get("location") or "").strip()[:300] or None
        try:
            hours_spent = float(data.get("hours_spent")) if data.get("hours_spent") not in (None, "") else None
        except (TypeError, ValueError):
            hours_spent = None
        # Для не-сборок — описание обязательно
        if not is_assembly and not description:
            return web.json_response(
                {"error": "no_description",
                 "message": "Для работ типа «" + db.WORK_TYPE_LABELS[work_type] + "» опишите что было сделано"},
                status=422,
            )
    ...
    serialized["assemblies"] = [
        {
            ...
            "status": a.get("status") or "in_progress",                                  # ЭТАП 18
            "status_label": db.ASSEMBLY_STATUS_LABELS.get(a.get("status") or "in_progress", "—"),  # ЭТАП 18
            "workers": [
...
async def update_contract(request: web.Request) -> web.Response:
    ...
    # ЭТАП 26.1: блокировка закрытия если не всё отгружено
    if data.get("status") == "closed":
        progress = await db.get_contract_shipment_progress(cid)
        total = int(progress.get("total") or 0)
        shipped = int(progress.get("shipped") or 0)
        force = bool(data.get("force_close"))
        # Есть позиции к отгрузке, но не всё отгружено — блокируем
        if total > 0 and shipped < total and not force:
            return web.json_response(
                {
                    "error": "shipment_incomplete",
                    "message": f"Нельзя закрыть договор: отгружено {shipped} из {total}. Сначала отгрузите остаток.",
                    "progress": {"total": total, "shipped": shipped},
                },
                status=409,
            )
        # Если позиций нет совсем (договор-услуга и т.п.) — пропускаем, фронт сам спросит подтверждение
...
async def create_employee_via_api(request: web.Request) -> web.Response:
    ...
    # ЭТАП 29: уровень доступа — приоритетный способ задания прав
    ...
    access_level = None
    if access_level_id:
        access_level = await db.get_access_level_by_id(access_level_id)
        if not access_level or not access_level.get("is_active"):
            return web.json_response(
                {"error": "validation", "message": "Уровень доступа не найден"},
                status=400,
            )

    # Роли (обратная совместимость — если фронт не прислал access_level_id)
    ...
    # Если ни уровня, ни ролей — ошибка
    if not access_level_id and not valid_roles:
        return web.json_response(
            {"error": "validation", "message": "Укажите уровень доступа"},
            status=400,
        )
    ...
    # ЭТАП 29: привязываем к уровню доступа если задан
    if access_level_id:
        await db.update_employee_access_level(new_id, access_level_id)

    # ЭТАП 28: если в запросе пришёл пароль — устанавливаем сразу
    password_raw = data.get("password")
    has_password = False
    if password_raw:
        password = str(password_raw).strip()
        if len(password) >= 6 and len(password) <= 100:
            # Проверим уникальность среди уже активных сотрудников с паролем
            duplicate = await db.verify_password_and_get_employee(password)
            if duplicate and duplicate.get("id") != new_id:
                # Не падаем — создание сотрудника прошло, просто пароль не установлен.
                # Директор может задать пароль позже через /api/employees/{id}/password
                pass
            else:
                await db.set_employee_password(new_id, password)
                has_password = True
    ...
    # Effective roles — derive из permissions уровня
    effective_perms = await db.get_employee_permissions(new_id)
    effective_roles = sorted(db.derive_legacy_roles(effective_perms)) if access_level_id else sorted(valid_roles)
    return web.json_response({
        "id": created["id"],
        ...
        "access_level_id": created.get("access_level_id"),
        "access_level_name": access_level["name"] if access_level else None,
    }, status=201)
...
async def update_employee_via_api(request: web.Request) -> web.Response:
    ...
    # ЭТАП 29: смена уровня доступа
    access_level_id_change = "access_level_id" in data
    new_access_level_id = None
    new_access_level = None
    if access_level_id_change:
        new_access_level_id = data.get("access_level_id")
        if new_access_level_id is not None:
            try:
                new_access_level_id = int(new_access_level_id)
            except (TypeError, ValueError):
                new_access_level_id = None
            if new_access_level_id:
                new_access_level = await db.get_access_level_by_id(new_access_level_id)
                if not new_access_level or not new_access_level.get("is_active"):
                    return web.json_response(
                        {"error": "validation", "message": "Уровень доступа не найден"},
                        status=400,
                    )

        # Защита: не оставить систему без hr.manage_access
        old_perms = await db.get_employee_permissions(eid)
        new_perms = set(new_access_level["permissions"]) if new_access_level else set()
        if "hr.manage_access" in old_perms and "hr.manage_access" not in new_perms:
            # Этот сотрудник терял право управлять доступом — проверим, не последний ли он
            total = await db.count_employees_with_perm("hr.manage_access")
            if total <= 1:
                return web.json_response(
                    {"error": "validation",
                     "message": "Нельзя — это последний сотрудник с правом «настраивать уровни доступа»"},
                    status=400,
                )
    ...
    # ЭТАП 29: применяем новый уровень доступа
    if access_level_id_change:
        await db.update_employee_access_level(eid, new_access_level_id)

    await db.log_action(chat_id, "update_employee", "employee", eid, "")

    # Обновляем кэш ролей если что-то менялось из влияющего на права
    if roles_str is not None or "telegram_id" in data or access_level_id_change:
        cache = await db.load_roles_to_cache()
        from .config import set_roles_cache
        set_roles_cache(cache)

    updated = await db.get_employee_by_id(eid)
    # Effective roles из permissions уровня доступа
    effective_perms = await db.get_employee_permissions(eid)
    if updated.get("access_level_id"):
        effective_roles = sorted(db.derive_legacy_roles(effective_perms))
    else:
        effective_roles = sorted({r.strip() for r in (updated.get("roles") or "").split(",") if r.strip()})
    access_level = None
    if updated.get("access_level_id"):
        access_level = await db.get_access_level_by_id(updated["access_level_id"])
    ...
    return web.json_response({
        ...
        "access_level_id": updated.get("access_level_id"),
        "access_level_name": access_level["name"] if access_level else None,
    })
...
# ЭТАП 28: установка/смена/сброс пароля сотрудника
@require_director
async def set_employee_password_via_api(request: web.Request) -> web.Response:
    """Устанавливает или сбрасывает пароль сотрудника. Только директор.

    Тело JSON:
        { "password": "abc12345" }        — установить новый пароль
        { "password": null }              — снять пароль (вход по паролю отключится)

    Проверки:
    - Минимум 6 символов
    - Пароль не должен совпадать с уже существующим у другого сотрудника
      (иначе вход по одному полю будет недетерминированным)
    """
    try:
        eid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
    ...
    if raw is None or raw == "":
        # Сброс пароля
        await db.clear_employee_password(eid)
        await db.log_action(chat_id, "clear_password", "employee", eid,
                            existing.get("full_name") or "")
        return web.json_response({"ok": True, "has_password": False})
    ...
    if len(password) > 100:
        return web.json_response(
            {"error": "validation",
             "message": "Пароль слишком длинный (макс 100 символов)"},
            status=400,
        )

    # Уникальность: если такой пароль уже у кого-то — отказ. Кроме случая, когда
    # это тот же сотрудник (т.е. директор «меняет» на то же самое).
    duplicate = await db.verify_password_and_get_employee(password)
    if duplicate and duplicate.get("id") != eid:
        return web.json_response(
            {"error": "validation",
             "message": "Такой пароль уже используется. Придумайте другой."},
            status=400,
        )

    ok = await db.set_employee_password(eid, password)
    if not ok:
        return web.json_response({"error": "internal"}, status=500)

    await db.log_action(chat_id, "set_password", "employee", eid,
                        existing.get("full_name") or "")

    # Кэш ролей — обновим, т.к. могли проставить auth_chat_id
    cache = await db.load_roles_to_cache()
    from .config import set_roles_cache
    set_roles_cache(cache)

    return web.json_response({"ok": True, "has_password": True})
...
def _serialize_position(p: dict, usage_count: int | None = None) -> dict:
    out = {
        "id": p["id"],
        "name": p.get("name") or "",
...
async def get_positions(request: web.Request) -> web.Response:
    """Список должностей. Доступен всем авторизованным — нужен для рендера формы сотрудника.
    Параметр ?include_inactive=true — для экрана управления (только директор увидит)."""
    include_inactive = request.query.get("include_inactive", "false").lower() == "true"
    rows = await db.get_positions(include_inactive=include_inactive)
    # Если запрошены и неактивные — это директор смотрит управление, добавим счётчики
    if include_inactive:
        result = []
        for p in rows:
            cnt = await db.count_employees_with_position(p["name"])
            result.append(_serialize_position(p, usage_count=cnt))
    else:
        result = [_serialize_position(p) for p in rows]
    return web.json_response({"positions": result})


@require_director
async def create_position_via_api(request: web.Request) -> web.Response:
    """Создаёт должность. Только директор."""
    try:
        data = await request.json()
    except json.JSONDecodeError:
        return web.json_response({"error": "bad_json"}, status=400)
    ...
    if not name:
        return web.json_response(
            {"error": "validation", "message": "Укажите название должности"},
            status=400,
        )
    if len(name) > 100:
        return web.json_response(
            {"error": "validation", "message": "Название слишком длинное (макс 100)"},
            status=400,
        )
    sort_order = data.get("sort_order")
    try:
        sort_order = int(sort_order) if sort_order is not None else 100
    except (TypeError, ValueError):
        sort_order = 100
    new_id = await db.create_position(name, sort_order)
    if not new_id:
        return web.json_response(
            {"error": "duplicate", "message": "Должность с таким названием уже есть"},
            status=400,
        )
    chat_id = request.get("chat_id")
    await db.log_action(chat_id, "create_position", "position", new_id, name)
    created = await db.get_position_by_id(new_id)
    return web.json_response(_serialize_position(created), status=201)


@require_director
async def update_position_via_api(request: web.Request) -> web.Response:
    """Обновляет должность. Только директор."""
    try:
        pid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)
    existing = await db.get_position_by_id(pid)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        data = await request.json()
    ...
    if name is not None:
        ...
        if not name:
            return web.json_response(
                {"error": "validation", "message": "Название не может быть пустым"},
                status=400,
            )
        if len(name) > 100:
            return web.json_response(
    ...
    if sort_order is not None:
        try:
            ...
            sort_order = None
    ok = await db.update_position(pid, name=name, sort_order=sort_order)
    if not ok:
        return web.json_response(
            {"error": "duplicate", "message": "Должность с таким названием уже есть"},
            status=400,
    ...
    await db.log_action(chat_id, "update_position", "position", pid,
                        (name or old_name))
    updated = await db.get_position_by_id(pid)
    return web.json_response(_serialize_position(updated))


@require_director
async def delete_position_via_api(request: web.Request) -> web.Response:
    """Soft-delete должности. Только директор. Если ?restore=true — восстанавливает."""
    try:
        pid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)
    ...
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    restore = request.query.get("restore", "false").lower() == "true"
    chat_id = request.get("chat_id")
    if restore:
        await db.restore_position(pid)
        await db.log_action(chat_id, "restore_position", "position", pid,
                            existing.get("name") or "")
        return web.json_response({"ok": True, "is_active": True})
    await db.delete_position(pid)
    await db.log_action(chat_id, "delete_position", "position", pid,
                        existing.get("name") or "")
    return web.json_response({"ok": True, "is_active": False})


# ============================================================================
# ЭТАП 29: УРОВНИ ДОСТУПА (ACCESS LEVELS)
# ============================================================================


async def _require_access_manage(request: web.Request) -> bool:
    """Проверка: есть ли у текущего пользователя право hr.manage_access.
    Загружает permissions на лету и кэширует в request.
    v2.9.4: использует get_effective_permissions — учитывает legacy-роль director
    (для владельца чата без access_level_id и аналогичных случаев)."""
    chat_id = request.get("chat_id")
    if not chat_id:
        return False
    if request.get("_perms") is None:
        roles = get_user_roles(chat_id) or set()
        request["_perms"] = await db.get_effective_permissions(chat_id, roles)
    return "hr.manage_access" in request["_perms"]


@require_auth
async def get_access_levels(request: web.Request) -> web.Response:
    """Список всех уровней доступа.

    Видит любой авторизованный (нужен для отображения должности в карточке сотрудника).
    ?include_inactive=true — только для тех у кого hr.manage_access.
    """
    include_inactive = request.query.get("include_inactive", "false").lower() == "true"
    if include_inactive and not await _require_access_manage(request):
        include_inactive = False

    levels = await db.get_access_levels(include_inactive=include_inactive)

    # Счётчики использования (только при include_inactive=true — это управление)
    if include_inactive and await _require_access_manage(request):
        for lvl in levels:
            lvl["usage_count"] = await db.count_employees_with_access_level(lvl["id"])

    return web.json_response({"levels": levels})


@require_auth
async def get_permissions_registry(request: web.Request) -> web.Response:
    """Список всех известных permission-ключей с label и группами.
    Нужен фронту для рендера экрана матрицы прав."""
    registry = [
        {"key": k, "label": label, "group": group}
        for (k, label, group) in db.PERMISSIONS_REGISTRY
    ]
    return web.json_response({"permissions": registry})


@require_auth
async def create_access_level_via_api(request: web.Request) -> web.Response:
    """Создаёт уровень. Только для hr.manage_access."""
    if not await _require_access_manage(request):
        return web.json_response({"error": "forbidden",
                                  "message": "Нет права настраивать уровни доступа"}, status=403)
    try:
        data = await request.json()
    except json.JSONDecodeError:
        return web.json_response({"error": "bad_json"}, status=400)
    ...
    name = (data.get("name") or "").strip()
    if not name:
        return web.json_response({"error": "validation",
                                  "message": "Укажите название уровня"}, status=400)
    if len(name) > 100:
        return web.json_response({"error": "validation",
                                  "message": "Название слишком длинное (макс 100)"}, status=400)

    perms = data.get("permissions") or []
    if not isinstance(perms, list):
        return web.json_response({"error": "validation",
                                  "message": "permissions должен быть массивом"}, status=400)
    ...
    new_id = await db.create_access_level(name, perms, sort_order)
    if not new_id:
        return web.json_response({"error": "duplicate",
                                  "message": "Уровень с таким названием уже есть"}, status=400)

    chat_id = request.get("chat_id")
    await db.log_action(chat_id, "create_access_level", "access_level", new_id, name)

    created = await db.get_access_level_by_id(new_id)
    return web.json_response(created, status=201)


@require_auth
async def update_access_level_via_api(request: web.Request) -> web.Response:
    if not await _require_access_manage(request):
        return web.json_response({"error": "forbidden",
                                  "message": "Нет права настраивать уровни доступа"}, status=403)
    try:
        lid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)

    existing = await db.get_access_level_by_id(lid)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    ...
    name = data.get("name")
    perms = data.get("permissions") if "permissions" in data else None
    sort_order = data.get("sort_order")

    # ЗАЩИТА: нельзя себе самому снять hr.manage_access если ты — последний с этим правом
    if perms is not None:
        new_perms_set = set(p for p in perms if isinstance(p, str))
        # Проверяем: если у текущего пользователя level == lid, и он снимает manage_access
        my_chat_id = request.get("chat_id")
        my_emp = await db.get_employee_by_telegram_id(my_chat_id)
        if not my_emp:
            my_emp = await db.get_employee_by_auth_chat_id(my_chat_id)
        if my_emp and my_emp.get("access_level_id") == lid:
            if "hr.manage_access" in (existing.get("permissions") or []) and \
               "hr.manage_access" not in new_perms_set:
                # Считаем сколько ещё людей с manage_access у других уровней
                total = await db.count_employees_with_perm("hr.manage_access")
                # total включает меня. Если я единственный — блокируем
                if total <= 1:
                    return web.json_response(
                        {"error": "validation",
                         "message": "Нельзя снять «настраивать уровни доступа» — вы единственный с этим правом"},
                        status=400,
                    )

    ok, err = await db.update_access_level(lid, name=name, permissions=perms, sort_order=sort_order)
    if not ok:
        msg = {
            "not_found": "Уровень не найден",
            "duplicate": "Уровень с таким названием уже есть",
            "system_locked": "Системный уровень нельзя переименовать",
        }.get(err, "Ошибка")
        return web.json_response({"error": err, "message": msg}, status=400)

    chat_id = request.get("chat_id")
    await db.log_action(chat_id, "update_access_level", "access_level", lid,
                        existing.get("name") or "")

    # Обновляем кэш ролей — у пользователей с этим уровнем могли поменяться effective roles
    cache = await db.load_roles_to_cache()
    from .config import set_roles_cache
    set_roles_cache(cache)

    updated = await db.get_access_level_by_id(lid)
    return web.json_response(updated)


@require_auth
async def delete_access_level_via_api(request: web.Request) -> web.Response:
    if not await _require_access_manage(request):
        return web.json_response({"error": "forbidden",
                                  "message": "Нет права настраивать уровни доступа"}, status=403)
    try:
    ...
    ok, err = await db.delete_access_level(lid)
    if not ok:
        msg = {
            "not_found": "Уровень не найден",
            "system_locked": "Системный уровень нельзя удалить",
            "in_use": "Уровень привязан к сотрудникам — сначала переназначьте их на другой",
        }.get(err, "Ошибка")
        return web.json_response({"error": err, "message": msg}, status=400)

    chat_id = request.get("chat_id")
    await db.log_action(chat_id, "delete_access_level", "access_level", lid,
                        existing.get("name") or "")

    return web.json_response({"ok": True, "is_active": False})


# ============ КОНЕЦ ЭТАПА 29: ENDPOINT'Ы УРОВНЕЙ ДОСТУПА ============
...
def _serialize_task(t: dict) -> dict:
    ...
    ЭТАП 16В-2: добавлены поля contract_id / contract_number / contractor_name
    для отображения бейджа "№ X · Контрагент" в списке и карточке.
    """
    return {
        "id": t["id"],
        ...
        # ЭТАП 16В-2 — связь с договором
        "contract_id": t.get("contract_id"),
        "contract_number": t.get("contract_number") or "",
        "contract_is_active": bool(t.get("contract_is_active")) if t.get("contract_id") else False,
        "contractor_name": t.get("contractor_name") or "",
...
async def get_tasks_endpoint(request: web.Request) -> web.Response:
    ...
    # ЭТАП 16В-2: фильтр по договору
    contract_id_q = q.get("contract_id")
    if contract_id_q:
        try:
            filter_kwargs["contract_id"] = int(contract_id_q)
        except (TypeError, ValueError):
            pass
...
async def create_task_endpoint(request: web.Request) -> web.Response:
    ...
    # ЭТАП 16В-2: опциональная привязка к договору
    contract_id = data.get("contract_id")
    if contract_id is not None and contract_id != "":
        try:
            contract_id = int(contract_id)
            c = await db.get_contract_by_id(contract_id) if hasattr(db, "get_contract_by_id") else None
            # Fallback: если функции get_contract_by_id нет — проверяем через _fetch_one
            if c is None:
                c = await db._fetch_one("SELECT * FROM contracts WHERE id = ?", (contract_id,))
            if not c or not c.get("is_active"):
                return web.json_response(
                    {"error": "validation", "message": "Договор не найден или архивирован"},
                    status=400,
                )
        except (TypeError, ValueError):
            return web.json_response({"error": "validation", "message": "Некорректный договор"}, status=400)
    else:
        contract_id = None
    ...
    new_id = await db.create_task(
        ...
        priority=priority,
        source=source,
        contract_id=contract_id,                # ЭТАП 16В-2
    )
...
async def update_task_endpoint(request: web.Request) -> web.Response:
    ...
    # ЭТАП 16В-2: смена/снятие привязки к договору
    if "contract_id" in data:
        cid_raw = data["contract_id"]
        if cid_raw is None or cid_raw == "":
            fields["contract_id"] = None
        else:
            try:
                cid = int(cid_raw)
                c = await db._fetch_one("SELECT * FROM contracts WHERE id = ?", (cid,))
                if not c or not c.get("is_active"):
                    return web.json_response(
                        {"error": "validation", "message": "Договор не найден или архивирован"},
                        status=400,
                    )
                fields["contract_id"] = cid
            except (TypeError, ValueError):
                return web.json_response({"error": "validation", "message": "Некорректный договор"}, status=400)
...
@require_auth
async def get_contract_tasks_endpoint(request: web.Request) -> web.Response:
    """Список задач, привязанных к договору.

    GET /api/contracts/{id}/tasks
    Возвращает все активные задачи этого договора + счётчики (open / done / total).
    """
    try:
        cid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
    ...
    contract = await db._fetch_one("SELECT * FROM contracts WHERE id = ?", (cid,))
    if not contract:
        return web.json_response({"error": "not_found"}, status=404)

    rows = await db.get_tasks(contract_id=cid)
    counts = await db.get_tasks_count_by_contract(cid)
    return web.json_response({
        "tasks": [_serialize_task(r) for r in rows],
        "counts": counts,
        "contract": {
            ...
            "is_active": bool(contract.get("is_active")),
        },
    })
...
#
# Эндпоинты:
# - GET    /api/warehouse/stock              — список сборок на складе с фильтрами
# - GET    /api/warehouse/stock/summary      — сводка (всего / свободных / зарезервированных)
# - GET    /api/warehouse/movements          — журнал движений
# - POST   /api/warehouse/movements          — создать движение вручную (приход / расход / списание)
# - GET    /api/assemblies/{id}/stock        — карточка остатка одной сборки + история
# - PATCH  /api/assemblies/{id}/status       — сменить статус сборки (с автохуком на движение)
# - POST   /api/contracts/{id}/ship          — массовая отгрузка готовых сборок по договору


def _serialize_stock_row(r: dict) -> dict:
    """Превращает row из get_warehouse_stock() в JSON для фронта."""
    return {
        "id":                 r["id"],
        "model_id":           r.get("model_id"),
        "model_name":         r.get("model_name") or "—",
        "model_article":         r.get("model_article") or "",
        "execution":          r.get("execution") or "",
        "ip_class":           r.get("ip_class") or "",
        "quantity":           r.get("quantity") or 1,
        "stock_qty":          int(r.get("stock_qty") or 0),
        "assembly_date":      r.get("assembly_date"),
        "status":             r.get("status") or "in_progress",
        "status_label":       db.ASSEMBLY_STATUS_LABELS.get(r.get("status") or "in_progress", "—"),
        "comment":            r.get("comment") or "",
        "contract_id":        r.get("contract_id"),
        "contract_number":    r.get("contract_number") or "",
        "contract_is_active": bool(r.get("contract_is_active")) if r.get("contract_id") else False,
        "contractor_name":    r.get("contractor_name") or "",
        "created_at":         r.get("created_at"),
...
def _serialize_movement(r: dict) -> dict:
    """Запись журнала движений в JSON."""
    return {
        "id":                 r["id"],
        "assembly_id":        r["assembly_id"],
        "direction":          r["direction"],
        "direction_label":    db.WAREHOUSE_DIRECTION_LABELS.get(r["direction"], r["direction"]),
        "qty":                r["qty"],
        "contract_id":        r.get("contract_id"),
        "contract_number":    r.get("contract_number") or "",
        "contractor_name":    r.get("contractor_name") or "",
        ...
        "comment":            r.get("comment") or "",
        "created_at":         r.get("created_at"),
        "created_by_chat_id": r.get("created_by_chat_id"),
        # Контекст сборки (для отображения в журнале без отдельного запроса)
        "model_name":         r.get("model_name") or "—",
        "model_article":         r.get("model_article") or "",
        "a_execution":        r.get("a_execution") or "",
        "a_ip_class":         r.get("a_ip_class") or "",
    }


@require_auth
async def get_warehouse_stock_endpoint(request: web.Request) -> web.Response:
    """Список сборок на складе.

    Query params:
      - search:        поиск по названию модели / исполнению / IP-классу
      - contract_id:   N или 'free' (без договора) или 'reserved' (с договором)
      - limit:         макс. число результатов (default 500)
    """
    q = request.query
    ...
    rows = await db.get_warehouse_stock(
        search=search,
        contract_id=contract_id,
        only_free=only_free,
    ...
    summary = await db.get_warehouse_stock_summary()
    return web.json_response({
        "stock":   [_serialize_stock_row(r) for r in rows],
        "summary": summary,
    })


@require_auth
async def get_warehouse_stock_summary_endpoint(request: web.Request) -> web.Response:
    """Только сводка — для лёгкого опроса (например, для шапки)."""
    summary = await db.get_warehouse_stock_summary()
    return web.json_response(summary)


@require_auth
async def get_warehouse_movements_endpoint(request: web.Request) -> web.Response:
    """Журнал движений склада.

    Query params: assembly_id, contract_id, direction, date_from, date_to, limit.
    """
    q = request.query
    kwargs: dict = {}

    if q.get("assembly_id"):
        try: kwargs["assembly_id"] = int(q["assembly_id"])
        except (TypeError, ValueError): pass
    if q.get("contract_id"):
        try: kwargs["contract_id"] = int(q["contract_id"])
        except (TypeError, ValueError): pass
    direction = q.get("direction")
    if direction in db.WAREHOUSE_DIRECTIONS:
        kwargs["direction"] = direction
    if q.get("date_from"):
        kwargs["date_from"] = q["date_from"]
    if q.get("date_to"):
        kwargs["date_to"] = q["date_to"]
    try:
        kwargs["limit"] = int(q.get("limit") or "200")
    except (TypeError, ValueError):
        kwargs["limit"] = 200

    rows = await db.get_warehouse_movements(**kwargs)
    return web.json_response({"movements": [_serialize_movement(r) for r in rows]})


@require_master_or_director
async def create_warehouse_movement_endpoint(request: web.Request) -> web.Response:
    """Создаёт ручное движение склада: приход / расход / списание.
    ...
    Тело (JSON):
      {
        ...
        "qty":         1,
        "contract_id": null | 12,    // обязательно для 'out' по договору
        "reason":      "брак",
        "comment":     "..."
      }

    Также автоматически меняет статус сборки:
      - 'in'        → не меняет статус (статус управляется отдельно)
      - 'out'       → assembly.status = 'shipped'
      - 'write_off' → assembly.status = 'written_off'
    """
    chat_id = request.get("chat_id")
    try:
        data = await request.json()
    ...
    # Валидация assembly_id
    aid_raw = data.get("assembly_id")
    try:
        aid = int(aid_raw)
    except (TypeError, ValueError):
        return web.json_response({"error": "validation", "message": "Укажите сборку"}, status=400)
    assembly = await db._fetch_one("SELECT * FROM assemblies WHERE id = ?", (aid,))
    if not assembly or not assembly.get("is_active"):
        return web.json_response({"error": "not_found", "message": "Сборка не найдена"}, status=404)

    # Валидация direction
    direction = data.get("direction")
    if direction not in db.WAREHOUSE_DIRECTIONS:
        return web.json_response(
            {"error": "validation", "message": "Направление должно быть in / out / write_off"},
            status=400,
        )
    ...
    try:
        ...
        return web.json_response({"error": "validation", "message": "Количество должно быть > 0"}, status=400)

    # contract_id для расхода (опционально, но для 'out' рекомендуем)
    contract_id = data.get("contract_id")
    if contract_id is not None and contract_id != "":
        try:
            contract_id = int(contract_id)
            ...
            if not c:
                return web.json_response(
                    {"error": "validation", "message": "Договор не найден"},
                    status=400,
                )
        except (TypeError, ValueError):
            return web.json_response({"error": "validation", "message": "Некорректный договор"}, status=400)
    ...
    # Проверка остатка для расхода / списания (защита от двойной отгрузки)
    if direction in ("out", "write_off"):
        current_stock = await db.get_assembly_stock_qty(aid)
        if current_stock < qty:
            return web.json_response(
                {"error": "validation",
                 "message": f"Недостаточно на складе (есть {current_stock}, нужно {qty})"},
                status=400,
            )
    ...
    # Создаём движение
    mov_id = await db.create_warehouse_movement(
        assembly_id=aid,
        direction=direction,
        qty=qty,
        contract_id=contract_id,
    ...
    # Хук: меняем статус сборки в зависимости от направления
    new_status = None
    if direction == "out":
        new_status = "shipped"
    elif direction == "write_off":
        new_status = "written_off"
    if new_status:
        async with __import__("aiosqlite").connect(db.DB_PATH) as conn:
            await conn.execute(
                """UPDATE assemblies
                   SET status = ?, updated_at = datetime('now'), updated_by_chat_id = ?
                   WHERE id = ?""",
                (new_status, chat_id, aid),
            )
            await conn.commit()

    await db.log_action(chat_id, f"warehouse_{direction}", "warehouse_movement", mov_id, f"assembly={aid}")
    return web.json_response({
        "movement_id": mov_id,
        "assembly_id": aid,
        "direction":   direction,
...
@require_auth
async def get_assembly_stock_endpoint(request: web.Request) -> web.Response:
    """Карточка остатка одной сборки + история движений."""
    try:
        aid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)

    # Базовая инфа о сборке
    row = await db._fetch_one(
        """SELECT a.*,
                  m.name AS model_name, m.article AS model_article,
                  c.number AS contract_number, c.is_active AS contract_is_active,
    ...
    qty = await db.get_assembly_stock_qty(aid)
    movements = await db.get_assembly_movements(aid)

    serialized = _serialize_stock_row({**row, "stock_qty": qty})
    return web.json_response({
        "assembly":  serialized,
...
@require_master_or_director
async def update_assembly_status_endpoint(request: web.Request) -> web.Response:
    """Меняет статус сборки. Может вызвать автосоздание движения склада.

    Тело: {"status": "in_progress" | "ready" | "shipped" | "written_off"}

    Логика автосоздания:
      - in_progress → ready:       автоматически создаётся 'in' (приход на склад)
      - ready       → shipped:     движение НЕ создаётся (ожидается явный 'out' через ship_contract)
      - ready       → written_off: автоматически создаётся 'write_off'
      - in_progress → shipped/written_off: НЕ разрешаем (нужно сначала ready)

    Для прямой отгрузки используй POST /api/warehouse/movements (direction='out'),
    он сам выставит статус 'shipped'.
    """
    chat_id = request.get("chat_id")
    try:
        aid = int(request.match_info.get("id", "0"))
    ...
    if new_status not in db.ASSEMBLY_STATUSES:
        return web.json_response(
            {"error": "validation", "message": "Статус должен быть in_progress / ready / shipped / written_off"},
            status=400,
        )

    old_status = assembly.get("status") or "in_progress"
    if new_status == old_status:
        return web.json_response({"assembly_id": aid, "status": new_status, "no_change": True})

    # Запрещаем прыжки in_progress → shipped/written_off без промежуточного ready
    if old_status == "in_progress" and new_status in ("shipped", "written_off"):
        return web.json_response(
            {"error": "validation",
             "message": "Сначала переведите сборку в статус 'Готова', потом отгружайте/списывайте"},
            status=400,
        )
    ...
    # Хук на автосоздание движения
    qty = assembly.get("quantity") or 1
    movement_id = None
    if old_status == "in_progress" and new_status == "ready":
        # Приход на склад — но только если для этой сборки ещё нет 'in'
        # (защита от повторного, например если статус был возвращён назад и снова на ready)
        existing_in = await db._fetch_one(
            "SELECT id FROM warehouse_movements WHERE assembly_id = ? AND direction = 'in' LIMIT 1",
            (aid,),
        )
        if not existing_in:
            movement_id = await db.create_warehouse_movement(
                assembly_id=aid,
                direction="in",
                qty=qty,
                reason="Сборка переведена в статус 'Готова'",
                created_by_chat_id=chat_id,
            )
    elif new_status == "written_off" and old_status == "ready":
        # Списание брака — создаём write_off, но только если остаток > 0
        current_stock = await db.get_assembly_stock_qty(aid)
        if current_stock > 0:
            movement_id = await db.create_warehouse_movement(
                assembly_id=aid,
    ...
    await db.log_action(chat_id, "assembly_status_change", "assembly", aid, f"{old_status} → {new_status}")
    return web.json_response({
        "assembly_id":   aid,
...
@require_sales_manage
async def ship_contract_endpoint(request: web.Request) -> web.Response:
    """Массовая отгрузка готовых сборок по договору.

    Берёт ВСЕ сборки с contract_id=N, status='ready', is_active=1
    и для каждой создаёт расход + меняет статус на 'shipped'.
    ...
    if not contract.get("is_active"):
        return web.json_response(
            {"error": "validation", "message": "Договор архивирован"},
            status=400,
        )

    result = await db.ship_assemblies_by_contract(contract_id=cid, chat_id=chat_id)
    await db.log_action(
        chat_id, "ship_by_contract", "contract", cid,
        f"shipped={result['shipped']}, skipped={result['skipped']}",
    )
...
#
# Эндпоинты:
# - /api/suppliers          GET/POST + /{id} GET/PATCH/DELETE
# - /api/supply-items       GET/POST + /{id} GET/PATCH/DELETE
# - /api/supply-requests    GET/POST + /{id} GET/PATCH/DELETE
# - /api/supply-orders      GET/POST + /{id} GET/PATCH
#                           + /{id}/items POST  (добавить позицию)
#                           + /items/{id} DELETE (удалить позицию)
#                           + /{id}/send  POST  (draft → sent)
# - /api/supply-receipts    GET/POST + /{id} GET
# - /api/supply/summary     GET  (сводка для шапки)


def require_supply_manage(handler):
    """Декоратор: эндпоинт для тех кто может работать в Снабжении.

    Это: директор, зам, менеджер (по образцу require_sales_manage).
    """
    async def wrapper(request: web.Request):
        chat_id = _get_chat_id_from_request(request)
        if chat_id is None:
        ...
        roles = get_user_roles(chat_id)
        if not (ROLE_DIRECTOR in roles or ROLE_ZAM in roles or ROLE_MANAGER in roles):
            return web.json_response(
                {"error": "forbidden",
                 "message": "Управление снабжением доступно директору, заму, менеджеру"},
                status=403,
            )
        request["chat_id"] = chat_id
        return await handler(request)
...
def _serialize_supply_item(i: dict) -> dict:
    return {
        "id":         i["id"],
        "name":       i.get("name") or "",
        "kind":       i.get("kind") or "material",
        "kind_label": db.SUPPLY_ITEM_KIND_LABELS.get(i.get("kind") or "material", "—"),
        "unit":       i.get("unit") or "шт.",
        "comment":    i.get("comment") or "",
        "is_active":  bool(i.get("is_active")),
        "created_at": i.get("created_at"),
        "updated_at": i.get("updated_at"),
    }


def _serialize_supply_request(r: dict) -> dict:
    return {
        "id":               r["id"],
        "item_id":          r.get("item_id"),
        "item_name":        r.get("item_name") or "",
        "item_kind":        r.get("item_kind") or "",
        "item_unit":        r.get("item_unit") or "шт.",
        ...
        "status_label":     db.SUPPLY_REQUEST_STATUS_LABELS.get(r.get("status") or "new", "—"),
        "comment":          r.get("comment") or "",
        "created_by_chat_id": r.get("created_by_chat_id"),
        "created_at":       r.get("created_at"),
        "updated_at":       r.get("updated_at"),
...
def _serialize_supply_order_item(oi: dict) -> dict:
    qty = float(oi.get("qty") or 0)
    price = oi.get("price")
    received = float(oi.get("received_qty") or 0)
    return {
        "id":            oi["id"],
        "item_id":       oi.get("item_id"),
        "item_name":     oi.get("item_name") or "",
        "item_kind":     oi.get("item_kind") or "",
        "item_unit":     oi.get("item_unit") or "шт.",
        "qty":           qty,
        "price":         float(price) if price is not None else None,
        "amount":        qty * (float(price) if price is not None else 0),
        "received_qty":  received,
        "request_id":    oi.get("request_id"),
        "request_needed_by": oi.get("request_needed_by"),
        "request_contract_id": oi.get("request_contract_id"),
        "contract_number": oi.get("contract_number") or "",
        "comment":       oi.get("comment") or "",
    }


def _serialize_supply_order(o: dict) -> dict:
    return {
        "id":            o["id"],
        "supplier_id":   o.get("supplier_id"),
        "supplier_name": o.get("supplier_name") or "",
        "supplier_contact": o.get("supplier_contact") or "",
        "supplier_phone":   o.get("supplier_phone") or "",
        "supplier_email":   o.get("supplier_email") or "",
        "status":        o.get("status") or "draft",
        "status_label":  db.SUPPLY_ORDER_STATUS_LABELS.get(o.get("status") or "draft", "—"),
        ...
        "items":         [_serialize_supply_order_item(it) for it in (o.get("items") or [])],
        "created_at":    o.get("created_at"),
...
def _serialize_supply_receipt(r: dict) -> dict:
    out = {
        "id":            r["id"],
        ...
        "received_by_chat_id": r.get("received_by_chat_id"),
        "created_at":    r.get("created_at"),
    }
    if "items" in r:
        out["items"] = [
            {
                ...
                "item_name":     ri.get("item_name") or "",
                "item_unit":     ri.get("item_unit") or "шт.",
                "item_kind":     ri.get("item_kind") or "",
                "qty":           float(ri.get("qty") or 0),
                "ordered_qty":   float(ri.get("ordered_qty") or 0),
                "price":         float(ri["price"]) if ri.get("price") is not None else None,
                "comment":       ri.get("comment") or "",
            }
            for ri in r["items"]
        ]
    return out
...
@require_auth
async def get_suppliers_endpoint(request: web.Request) -> web.Response:
    q = request.query
    search = (q.get("search") or "").strip() or None
    only_active = q.get("all") != "1"
    rows = await db.get_suppliers(only_active=only_active, search=search)
    return web.json_response({"suppliers": [_serialize_supplier(r) for r in rows]})


@require_supply_manage
async def create_supplier_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        data = await request.json()
    except json.JSONDecodeError:
    ...
    name = (data.get("name") or "").strip()
    if not name:
        return web.json_response({"error": "validation", "message": "Укажите название поставщика"}, status=400)
    new_id = await db.create_supplier(
        name=name,
    ...
    await db.log_action(chat_id, "create_supplier", "supplier", new_id, name)
    created = await db.get_supplier_by_id(new_id)
    return web.json_response(_serialize_supplier(created), status=201)


@require_auth
async def get_supplier_endpoint(request: web.Request) -> web.Response:
    try:
        sid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    row = await db.get_supplier_by_id(sid)
    if not row:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response(_serialize_supplier(row))


@require_supply_manage
async def update_supplier_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        sid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    existing = await db.get_supplier_by_id(sid)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        data = await request.json()
        ...
        return web.json_response({"error": "bad_json"}, status=400)
    fields: dict = {}
    for k in ("name", "inn", "contact_person", "phone", "email", "comment"):
        if k in data:
            fields[k] = (data[k] or "").strip() or None
    if "name" in fields and not fields["name"]:
        return web.json_response({"error": "validation", "message": "Название не может быть пустым"}, status=400)
    fields["updated_by_chat_id"] = chat_id
    await db.update_supplier(sid, **fields)
    await db.log_action(chat_id, "update_supplier", "supplier", sid, "")
    updated = await db.get_supplier_by_id(sid)
    return web.json_response(_serialize_supplier(updated))


@require_supply_manage
async def delete_supplier_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        sid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
    ...
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    await db.set_supplier_active(sid, False)
    await db.log_action(chat_id, "delete_supplier", "supplier", sid, existing.get("name") or "")
    return web.json_response({"deleted": True})
...
@require_auth
async def get_supply_items_endpoint(request: web.Request) -> web.Response:
    q = request.query
    search = (q.get("search") or "").strip() or None
    kind = q.get("kind") if q.get("kind") in db.SUPPLY_ITEM_KINDS else None
    only_active = q.get("all") != "1"
    rows = await db.get_supply_items(only_active=only_active, kind=kind, search=search)
    return web.json_response({"items": [_serialize_supply_item(r) for r in rows]})


@require_supply_manage
async def create_supply_item_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        data = await request.json()
    except json.JSONDecodeError:
    ...
    name = (data.get("name") or "").strip()
    if not name:
        return web.json_response({"error": "validation", "message": "Укажите название позиции"}, status=400)
    kind = data.get("kind") or "material"
    if kind not in db.SUPPLY_ITEM_KINDS:
        return web.json_response({"error": "validation", "message": "Тип позиции должен быть material или product"}, status=400)
    new_id = await db.create_supply_item(
        name=name,
        kind=kind,
    ...
    await db.log_action(chat_id, "create_supply_item", "supply_item", new_id, name)
    created = await db.get_supply_item_by_id(new_id)
    return web.json_response(_serialize_supply_item(created), status=201)


@require_auth
async def get_supply_item_endpoint(request: web.Request) -> web.Response:
    try:
        iid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    row = await db.get_supply_item_by_id(iid)
    if not row:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response(_serialize_supply_item(row))


@require_supply_manage
async def update_supply_item_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        iid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    existing = await db.get_supply_item_by_id(iid)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        data = await request.json()
    ...
    if "name" in data:
        ...
        fields["name"] = name
    if "kind" in data:
        if data["kind"] not in db.SUPPLY_ITEM_KINDS:
            return web.json_response({"error": "validation", "message": "Некорректный тип"}, status=400)
        fields["kind"] = data["kind"]
    if "unit" in data:
        fields["unit"] = (data["unit"] or "шт.").strip() or "шт."
    if "comment" in data:
        fields["comment"] = (data["comment"] or "").strip() or None
    await db.update_supply_item(iid, **fields)
    await db.log_action(chat_id, "update_supply_item", "supply_item", iid, "")
    updated = await db.get_supply_item_by_id(iid)
    return web.json_response(_serialize_supply_item(updated))


@require_supply_manage
async def delete_supply_item_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        iid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
    ...
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    await db.set_supply_item_active(iid, False)
    await db.log_action(chat_id, "delete_supply_item", "supply_item", iid, existing.get("name") or "")
    return web.json_response({"deleted": True})
...
@require_auth
async def get_supply_requests_endpoint(request: web.Request) -> web.Response:
    q = request.query
    status = q.get("status")
    contract_id = None
    if q.get("contract_id"):
        try: contract_id = int(q["contract_id"])
        except (TypeError, ValueError): pass
    rows = await db.get_supply_requests(status=status, contract_id=contract_id)
    return web.json_response({"requests": [_serialize_supply_request(r) for r in rows]})


@require_supply_manage
async def create_supply_request_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        data = await request.json()
    except json.JSONDecodeError:
    ...
    try:
        ...
        return web.json_response({"error": "validation", "message": "Укажите позицию"}, status=400)
    item = await db.get_supply_item_by_id(item_id)
    if not item or not item.get("is_active"):
        return web.json_response({"error": "validation", "message": "Позиция не найдена или архивирована"}, status=400)
    # qty
    try:
    ...
    if contract_id is not None and contract_id != "":
        try:
            ...
            c = await db._fetch_one("SELECT * FROM contracts WHERE id = ?", (contract_id,))
            if not c or not c.get("is_active"):
                return web.json_response({"error": "validation", "message": "Договор не найден или архивирован"}, status=400)
        except (TypeError, ValueError):
            return web.json_response({"error": "validation", "message": "Некорректный договор"}, status=400)
    else:
        contract_id = None
    new_id = await db.create_supply_request(
        item_id=item_id, qty=qty,
        needed_by=needed_by, contract_id=contract_id,
        comment=(data.get("comment") or "").strip() or None,
        created_by_chat_id=chat_id,
    )
    await db.log_action(chat_id, "create_supply_request", "supply_request", new_id, item.get("name") or "")
    created = await db.get_supply_request_by_id(new_id)
    return web.json_response(_serialize_supply_request(created), status=201)


@require_auth
async def get_supply_request_endpoint(request: web.Request) -> web.Response:
    try:
        rid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    row = await db.get_supply_request_by_id(rid)
    if not row:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response(_serialize_supply_request(row))


@require_supply_manage
async def update_supply_request_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        rid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    existing = await db.get_supply_request_by_id(rid)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        data = await request.json()
    ...
    if "item_id" in data:
        try:
            fields["item_id"] = int(data["item_id"])
        except (TypeError, ValueError):
            return web.json_response({"error": "validation", "message": "Некорректная позиция"}, status=400)
    if "qty" in data:
        try:
            q = float(data["qty"])
            if q <= 0:
    ...
    if "needed_by" in data:
        fields["needed_by"] = data["needed_by"] or None
    if "contract_id" in data:
        ...
        if cid is None or cid == "":
            ...
            try:
                fields["contract_id"] = int(cid)
            except (TypeError, ValueError):
                return web.json_response({"error": "validation", "message": "Некорректный договор"}, status=400)
    if "status" in data:
        if data["status"] not in db.SUPPLY_REQUEST_STATUSES:
            return web.json_response({"error": "validation", "message": "Некорректный статус"}, status=400)
        fields["status"] = data["status"]
    if "comment" in data:
        fields["comment"] = (data["comment"] or "").strip() or None
    await db.update_supply_request(rid, **fields)
    await db.log_action(chat_id, "update_supply_request", "supply_request", rid, "")
    updated = await db.get_supply_request_by_id(rid)
    return web.json_response(_serialize_supply_request(updated))


@require_supply_manage
async def delete_supply_request_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        rid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
    ...
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    await db.set_supply_request_active(rid, False)
    await db.log_action(chat_id, "delete_supply_request", "supply_request", rid, "")
    return web.json_response({"deleted": True})
...
@require_auth
async def get_supply_orders_endpoint(request: web.Request) -> web.Response:
    q = request.query
    status = q.get("status")
    supplier_id = None
    if q.get("supplier_id"):
        try: supplier_id = int(q["supplier_id"])
        except (TypeError, ValueError): pass
    rows = await db.get_supply_orders(status=status, supplier_id=supplier_id)
    return web.json_response({"orders": [_serialize_supply_order(r) for r in rows]})


@require_supply_manage
async def create_supply_order_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        data = await request.json()
    except json.JSONDecodeError:
    ...
    try:
        ...
        return web.json_response({"error": "validation", "message": "Укажите поставщика"}, status=400)
    sup = await db.get_supplier_by_id(supplier_id)
    if not sup or not sup.get("is_active"):
        return web.json_response({"error": "validation", "message": "Поставщик не найден или архивирован"}, status=400)
    new_id = await db.create_supply_order(
        supplier_id=supplier_id,
        expected_date=data.get("expected_date") or None,
        comment=(data.get("comment") or "").strip() or None,
        created_by_chat_id=chat_id,
    )
    await db.log_action(chat_id, "create_supply_order", "supply_order", new_id, sup.get("name") or "")
    created = await db.get_supply_order_by_id(new_id)
    return web.json_response(_serialize_supply_order(created), status=201)


@require_auth
async def get_supply_order_endpoint(request: web.Request) -> web.Response:
    try:
        oid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    row = await db.get_supply_order_by_id(oid)
    if not row:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response(_serialize_supply_order(row))


@require_supply_manage
async def update_supply_order_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        oid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    existing = await db.get_supply_order_by_id(oid)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        data = await request.json()
    ...
    if "supplier_id" in data:
        try:
            ...
            return web.json_response({"error": "validation", "message": "Некорректный поставщик"}, status=400)
        s = await db.get_supplier_by_id(sid)
        if not s:
            return web.json_response({"error": "validation", "message": "Поставщик не найден"}, status=400)
        fields["supplier_id"] = sid
    if "status" in data:
        if data["status"] not in db.SUPPLY_ORDER_STATUSES:
            return web.json_response({"error": "validation", "message": "Некорректный статус"}, status=400)
        fields["status"] = data["status"]
    if "expected_date" in data:
        fields["expected_date"] = data["expected_date"] or None
    if "comment" in data:
        fields["comment"] = (data["comment"] or "").strip() or None
    await db.update_supply_order(oid, **fields)
    await db.log_action(chat_id, "update_supply_order", "supply_order", oid, "")
    updated = await db.get_supply_order_by_id(oid)
    return web.json_response(_serialize_supply_order(updated))


@require_supply_manage
async def add_supply_order_item_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        oid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    order = await db.get_supply_order_by_id(oid)
    if not order:
        return web.json_response({"error": "not_found"}, status=404)
    if order.get("status") not in ("draft", "sent", "partial"):
        return web.json_response({"error": "validation", "message": "В принятый или отменённый заказ нельзя добавлять позиции"}, status=400)
    try:
        data = await request.json()
    except json.JSONDecodeError:
        return web.json_response({"error": "bad_json"}, status=400)
    ...
    item = await db.get_supply_item_by_id(item_id)
    if not item:
        return web.json_response({"error": "validation", "message": "Позиция не найдена"}, status=400)
    try:
        qty = float(data.get("qty") or 0)
        if qty <= 0:
            raise ValueError
    ...
    if request_id is not None and request_id != "":
        try:
            ...
            req = await db.get_supply_request_by_id(request_id)
            if not req:
                return web.json_response({"error": "validation", "message": "Заявка не найдена"}, status=400)
        except (TypeError, ValueError):
            return web.json_response({"error": "validation", "message": "Некорректная заявка"}, status=400)
    else:
        request_id = None
    new_oi = await db.add_supply_order_item(
        order_id=oid, item_id=item_id, qty=qty, price=price,
        request_id=request_id,
        comment=(data.get("comment") or "").strip() or None,
    )
    await db.log_action(chat_id, "add_supply_order_item", "supply_order_item", new_oi, "")
    updated = await db.get_supply_order_by_id(oid)
    return web.json_response(_serialize_supply_order(updated), status=201)


@require_supply_manage
async def remove_supply_order_item_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        oi_id = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    ok = await db.remove_supply_order_item(oi_id)
    if not ok:
        return web.json_response({"error": "not_found"}, status=404)
    await db.log_action(chat_id, "remove_supply_order_item", "supply_order_item", oi_id, "")
    return web.json_response({"deleted": True})


@require_supply_manage
async def send_supply_order_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        oid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
    ...
    if order.get("status") != "draft":
        return web.json_response({"error": "validation", "message": "Отправить можно только заказ в статусе 'черновик'"}, status=400)
    if not order.get("items"):
        return web.json_response({"error": "validation", "message": "В заказе нет позиций"}, status=400)
    await db.send_supply_order(oid)
    await db.log_action(chat_id, "send_supply_order", "supply_order", oid, "")
    updated = await db.get_supply_order_by_id(oid)
    return web.json_response(_serialize_supply_order(updated))
...
@require_auth
async def get_supply_receipts_endpoint(request: web.Request) -> web.Response:
    q = request.query
    order_id = None
    if q.get("order_id"):
        try: order_id = int(q["order_id"])
        except (TypeError, ValueError): pass
    rows = await db.get_supply_receipts(
        order_id=order_id,
        date_from=q.get("date_from"),
        date_to=q.get("date_to"),
    )
    return web.json_response({"receipts": [_serialize_supply_receipt(r) for r in rows]})


@require_supply_manage
async def create_supply_receipt_endpoint(request: web.Request) -> web.Response:
    """Создаёт приёмку.
    ...
    Тело JSON:
    {
      ...
      "comment": "...",
      "items": [
        {"order_item_id": 42, "qty": 50.0, "comment": ""},
        {"order_item_id": 43, "qty": 100.0}
      ]
    }
    """
    chat_id = request.get("chat_id")
    ...
    try:
        ...
        return web.json_response({"error": "validation", "message": "Укажите заказ"}, status=400)
    order = await db.get_supply_order_by_id(order_id)
    if not order:
        return web.json_response({"error": "validation", "message": "Заказ не найден"}, status=400)
    if order.get("status") in ("draft", "cancelled"):
        return web.json_response({"error": "validation",
                                  "message": "Сначала отправьте заказ поставщику (или отмена не подходит)"}, status=400)
    received_date = data.get("received_date") or None
    if not received_date:
        return web.json_response({"error": "validation", "message": "Укажите дату приёмки"}, status=400)
    items = data.get("items") or []
    if not isinstance(items, list) or not items:
        return web.json_response({"error": "validation", "message": "Укажите хотя бы одну позицию"}, status=400)
    # Валидация позиций — каждая должна принадлежать заказу + qty > 0
    order_item_ids = {oi["id"] for oi in (order.get("items") or [])}
    cleaned = []
    for it in items:
        try:
            oi_id = int(it.get("order_item_id"))
            if oi_id not in order_item_ids:
                return web.json_response({"error": "validation",
                                          "message": f"Позиция {oi_id} не входит в заказ {order_id}"}, status=400)
            qty = float(it.get("qty") or 0)
            if qty <= 0:
                continue
        except (TypeError, ValueError):
            return web.json_response({"error": "validation", "message": "Некорректные данные позиции"}, status=400)
        cleaned.append({"order_item_id": oi_id, "qty": qty, "comment": it.get("comment") or None})
    if not cleaned:
        return web.json_response({"error": "validation", "message": "Нет позиций с количеством > 0"}, status=400)
    new_id = await db.create_supply_receipt(
        order_id=order_id, received_date=received_date,
        items=cleaned,
        comment=(data.get("comment") or "").strip() or None,
        received_by_chat_id=chat_id,
    )
    await db.log_action(chat_id, "create_supply_receipt", "supply_receipt", new_id,
                        f"order={order_id}, items={len(cleaned)}")
    created = await db.get_supply_receipt_by_id(new_id)
    return web.json_response(_serialize_supply_receipt(created), status=201)


@require_auth
async def get_supply_receipt_endpoint(request: web.Request) -> web.Response:
    try:
        rid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    row = await db.get_supply_receipt_by_id(rid)
    if not row:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response(_serialize_supply_receipt(row))


# ============ Сводка ============

@require_auth
async def get_supply_summary_endpoint(request: web.Request) -> web.Response:
    summary = await db.get_supply_summary()
    return web.json_response(summary)


# ============ ЭТАП 20: КАДРЫ — ОТПУСКА ============
# Эндпоинты:
# - GET    /api/vacations              (любой авторизованный — смотрит)
# - POST   /api/vacations              (только бухгалтер + директор)
# - PATCH  /api/vacations/{id}         (бухгалтер + директор)
# - DELETE /api/vacations/{id}         (бухгалтер + директор)
# - GET    /api/vacations/current      (текущие отпуска — для виджета Главной)
# - GET    /api/vacations/upcoming     (ближайшие отпуска)


def require_hr_manage(handler):
    """Декоратор: только бухгалтер или директор.

    Кадры (отпуска) ведёт бухгалтер. Директор — на всякий случай.
    """
    async def wrapper(request: web.Request):
        chat_id = _get_chat_id_from_request(request)
        if chat_id is None:
            return web.json_response({"error": "unauthorized"}, status=401)
        roles = get_user_roles(chat_id)
        if not (ROLE_DIRECTOR in roles or ROLE_ACCOUNTANT in roles):
            return web.json_response(
                {"error": "forbidden",
                 "message": "Управление кадрами доступно бухгалтеру или директору"},
                status=403,
            )
        request["chat_id"] = chat_id
        return await handler(request)
...
def _serialize_vacation(v: dict) -> dict:
    """Сериализатор отпуска. Также вычисляет days_total и status."""
    start = v.get("start_date") or ""
    end = v.get("end_date") or ""
    days = 0
    status = "planned"  # planned | current | finished
    try:
    ...
    return {
        ...
        "employee_full_name":  v.get("employee_full_name") or "",
        "employee_short_name": v.get("employee_short_name") or "",
        "employee_position":   v.get("employee_position") or "",
        "start_date":        start,
        "end_date":          end,
        "days_total":        days,
        "status":            status,
...
@require_auth
async def get_vacations_endpoint(request: web.Request) -> web.Response:
    q = request.query
    employee_id = None
    if q.get("employee_id"):
        try: employee_id = int(q["employee_id"])
        except (TypeError, ValueError): pass
    rows = await db.get_vacations(
        employee_id=employee_id,
        date_from=q.get("date_from"),
        date_to=q.get("date_to"),
    )
    return web.json_response({"vacations": [_serialize_vacation(r) for r in rows]})


@require_hr_manage
async def create_vacation_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        data = await request.json()
    except json.JSONDecodeError:
    ...
    try:
        ...
        return web.json_response({"error": "validation", "message": "Выберите сотрудника"}, status=400)
    emp = await db.get_employee_by_id(employee_id)
    if not emp:
        return web.json_response({"error": "validation", "message": "Сотрудник не найден"}, status=400)
    start_date = (data.get("start_date") or "").strip()
    end_date   = (data.get("end_date") or "").strip()
    if not start_date or not end_date:
        return web.json_response({"error": "validation", "message": "Укажите даты начала и конца"}, status=400)
    if start_date > end_date:
        return web.json_response({"error": "validation", "message": "Дата начала позже даты конца"}, status=400)
    new_id = await db.create_vacation(
        employee_id=employee_id,
        start_date=start_date, end_date=end_date,
        comment=(data.get("comment") or "").strip() or None,
        created_by_chat_id=chat_id,
    )
    await db.log_action(chat_id, "create_vacation", "vacation", new_id,
                        f"{emp.get('full_name','')}: {start_date}..{end_date}")
    created = await db.get_vacation_by_id(new_id)
    return web.json_response(_serialize_vacation(created), status=201)


@require_hr_manage
async def update_vacation_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        vid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    existing = await db.get_vacation_by_id(vid)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        data = await request.json()
    ...
    if "employee_id" in data:
        try:
            fields["employee_id"] = int(data["employee_id"])
        except (TypeError, ValueError):
            return web.json_response({"error": "validation", "message": "Некорректный сотрудник"}, status=400)
    if "start_date" in data:
        fields["start_date"] = (data["start_date"] or "").strip()
    if "end_date" in data:
        fields["end_date"] = (data["end_date"] or "").strip()
    if "comment" in data:
        fields["comment"] = (data["comment"] or "").strip() or None
    # Валидация: даты не пересекаются
    sd = fields.get("start_date", existing.get("start_date"))
    ed = fields.get("end_date",   existing.get("end_date"))
    if sd and ed and sd > ed:
        return web.json_response({"error": "validation", "message": "Дата начала позже даты конца"}, status=400)
    await db.update_vacation(vid, **fields)
    await db.log_action(chat_id, "update_vacation", "vacation", vid, "")
    updated = await db.get_vacation_by_id(vid)
    return web.json_response(_serialize_vacation(updated))


@require_hr_manage
async def delete_vacation_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        vid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
    ...
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    await db.set_vacation_active(vid, False)
    await db.log_action(chat_id, "delete_vacation", "vacation", vid, "")
    return web.json_response({"deleted": True})


@require_auth
async def get_current_vacations_endpoint(request: web.Request) -> web.Response:
    """Кто сейчас в отпуске + кто уходит в ближайшие 14 дней — для виджета на Главной."""
    current = await db.get_current_vacations()
    upcoming = await db.get_upcoming_vacations(days_ahead=14)
    return web.json_response({
        "current":  [_serialize_vacation(r) for r in current],
        "upcoming": [_serialize_vacation(r) for r in upcoming],
    })
...
# Эндпоинты БЕЗ авторизации. Доступ — только по непредсказуемому токену
# (secrets.token_urlsafe(8) ~ 64 бит энтропии, перебрать нереально).


async def get_public_assembly_endpoint(request: web.Request) -> web.Response:
    """Публичная страница сборки. Возвращает читаемый минимум.

    Включает: модель + артикул + исполнение + IP + дата + сборщики
    + договор/контрагент (если есть) + комментарий + статус.
    """
    token = (request.match_info.get("token") or "").strip()
    if not token:
        return web.json_response({"error": "bad_token"}, status=400)
    asm = await db.get_assembly_by_public_token(token)
    if not asm:
        return web.json_response({"error": "not_found"}, status=404)

    status_label = db.ASSEMBLY_STATUS_LABELS.get(asm.get("status") or "ready", "—") \
        if hasattr(db, "ASSEMBLY_STATUS_LABELS") else (asm.get("status") or "—")

    # История движений
    movements = []
    try:
        rows = await db.get_assembly_movements(asm["id"])
        for r in rows:
            dir_label = "приход" if r.get("direction") == "in" else (
                "расход" if r.get("direction") == "out" else "списание"
            )
            movements.append({
    ...
    return web.json_response({
        "id":              asm["id"],
        "model_name":      asm.get("model_name") or "",
        "model_article":   asm.get("model_article") or "",
        "execution":       asm.get("execution") or "",
        "ip_class":        asm.get("ip_class") or "",
        "quantity":        int(asm.get("quantity") or 0),
        "assembly_date":   asm.get("assembly_date") or "",
        "comment":         asm.get("comment") or "",
        "status":          asm.get("status") or "",
        "status_label":    status_label,
        "contract_id":     asm.get("contract_id"),
        "contract_number": asm.get("contract_number") or "",
        "contract_delivery_date": asm.get("contract_delivery_date") or "",
        "contractor_name": asm.get("contractor_name") or "",
        "workers": [
            {
                "id":         w["id"],
                "full_name":  w.get("full_name") or "",
                "short_name": w.get("short_name") or "",
            } for w in (asm.get("workers") or [])
        ],
        "movements": movements,
    })


async def get_public_contract_endpoint(request: web.Request) -> web.Response:
    """Публичная страница договора. Возвращает базовую инфу + список сборок."""
    token = (request.match_info.get("token") or "").strip()
    if not token:
        return web.json_response({"error": "bad_token"}, status=400)
    c = await db.get_contract_by_public_token(token)
    if not c:
        return web.json_response({"error": "not_found"}, status=404)
    ...
    assemblies = []
    for a in (c.get("assemblies") or []):
        assemblies.append({
            "id":             a["id"],
            "model_name":     a.get("model_name") or "",
            "model_article":  a.get("model_article") or "",
            "execution":      a.get("execution") or "",
            "ip_class":       a.get("ip_class") or "",
            "quantity":       int(a.get("quantity") or 0),
            "assembly_date":  a.get("assembly_date") or "",
            "status":         a.get("status") or "",
            "status_label":   asm_status_labels.get(a.get("status") or "", "—"),
        })

    return web.json_response({
        "id":              c["id"],
        "number":          c.get("number") or "",
        "sign_date":       c.get("sign_date") or "",
        "contractor_name": c.get("contractor_name") or "",
        "manager_name":    c.get("manager_short_name") or c.get("manager_name") or "",
        "status":          status,
        "status_label":    status_labels.get(status, status),
        "delivery_date":   c.get("delivery_date") or "",
        "delivery_address": c.get("delivery_address") or "",
...
@require_auth
async def get_assembly_token_endpoint(request: web.Request) -> web.Response:
    """Возвращает public_token сборки — для генерации QR на фронте.

    Если у сборки токена нет (старые до миграции) — создаст и вернёт.
    """
    try:
        aid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    token = await db.ensure_assembly_public_token(aid)
    if not token:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response({"id": aid, "public_token": token})


@require_auth
async def get_contract_token_endpoint(request: web.Request) -> web.Response:
    """Возвращает public_token договора — для генерации QR."""
    try:
        cid = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    token = await db.ensure_contract_public_token(cid)
    if not token:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response({"id": cid, "public_token": token})


# ============ ЭТАП 22: ДОРАБОТКИ — ЗАМЕЧАНИЯ ============


def require_defects_manage(handler):
    """Декоратор: только директор или зам — для смены статуса замечания."""
    async def wrapper(request: web.Request):
        chat_id = _get_chat_id_from_request(request)
        if chat_id is None:
            return web.json_response({"error": "unauthorized"}, status=401)
        roles = get_user_roles(chat_id)
        if not (ROLE_DIRECTOR in roles or ROLE_ZAM in roles):
            return web.json_response(
                {"error": "forbidden",
                 "message": "Управлять замечаниями могут только директор или зам"},
                status=403,
            )
        request["chat_id"] = chat_id
        return await handler(request)
...
def _serialize_defect(d: dict) -> dict:
    """Сериализация замечания."""
    type_label = db.DEFECT_TYPE_LABELS.get(d.get("type") or "defect", "—")
    status_label = db.DEFECT_STATUS_LABELS.get(d.get("status") or "new", "—")
    photos_in = d.get("photos") or []
    photos = []
    for p in photos_in:
        photos.append({
            "id":           p["id"],
            "url":          "/api/defects/photos/" + str(p["id"]),
            "file_size":    p.get("file_size") or 0,
            "content_type": p.get("content_type") or "",
        })
    return {
        "id":             d["id"],
        "type":           d.get("type") or "defect",
        "type_label":     type_label,
        "status":         d.get("status") or "new",
        "status_label":   status_label,
        "description":    d.get("description") or "",
        "author_name":    d.get("author_name") or "",
        "author_phone":   d.get("author_phone") or "",
        "location":       d.get("location") or "",
        "resolution_note": d.get("resolution_note") or "",
        "assembly_id":    d.get("assembly_id"),
        "assembly_token": d.get("assembly_token") or "",
        "model_name":     d.get("model_name") or "",
        "model_article":  d.get("model_article") or "",
        "contract_id":    d.get("contract_id"),
        "contract_number": d.get("contract_number") or "",
        "contract_token": d.get("contract_token") or "",
        "contractor_name": d.get("contractor_name") or "",
        "assignee_id":    d.get("assignee_id"),
        "assignee_name":  d.get("assignee_short_name") or d.get("assignee_full_name") or "",
        "created_at":     d.get("created_at"),
        "updated_at":     d.get("updated_at"),
        "photos":         photos,
        "photos_count":   int(d.get("photos_count") or len(photos)),
    }


async def create_defect_report_endpoint(request: web.Request) -> web.Response:
    """ПУБЛИЧНЫЙ эндпоинт создания замечания (БЕЗ авторизации).

    Принимает multipart/form-data:
    - description (text, обязательно)
    - type (defect|issue|improvement|question)
    - assembly_token или contract_token (хотя бы один)
    - author_name, author_phone, location (опционально)
    - photo_1...photo_5 — фото (до 5 шт, до 8 МБ каждое)
    """
    try:
        reader = await request.multipart()
    except Exception:
        return web.json_response({"error": "bad_request", "message": "Ожидается multipart/form-data"}, status=400)
    ...
    async for part in reader:
        if part is None:
            break
        name = part.name
        if name in ("photo_1", "photo_2", "photo_3", "photo_4", "photo_5", "photo"):
            ct = part.headers.get("Content-Type", "")
            filename = part.filename or "photo.bin"
            # Читаем по чанкам с проверкой размера
            data = bytearray()
            size = 0
            ...
            while chunk:
                ...
                if size > MAX_PHOTO_SIZE:
                    return web.json_response(
                        {"error": "photo_too_large", "message": f"Фото слишком большое (макс {MAX_PHOTO_SIZE // 1024 // 1024} МБ)"},
                        status=400,
                    )
                data.extend(chunk)
                chunk = await part.read_chunk(64 * 1024)
            if size == 0:
                continue
            if ct not in ALLOWED_PHOTO_TYPES:
                return web.json_response(
                    {"error": "bad_photo_type", "message": f"Тип файла не поддерживается: {ct}"},
                    status=400,
                )
            files.append({"name": name, "filename": filename, "content_type": ct, "data": bytes(data), "size": size})
        else:
    ...
    if not description:
        return web.json_response({"error": "validation", "message": "Опишите замечание"}, status=400)
    if len(description) > 5000:
        return web.json_response({"error": "validation", "message": "Описание слишком длинное"}, status=400)
    if len(files) > MAX_PHOTOS_PER_DEFECT:
        return web.json_response(
            {"error": "validation", "message": f"Максимум {MAX_PHOTOS_PER_DEFECT} фото"},
            status=400,
        )

    type_ = (fields.get("type") or "defect").strip()
    if type_ not in db.DEFECT_TYPE_LABELS:
        type_ = "defect"

    # Распознаём токен сборки/договора
    assembly_id = None
    contract_id = None
    asm_token = (fields.get("assembly_token") or "").strip()
    c_token   = (fields.get("contract_token") or "").strip()
    is_general = (fields.get("general") or "").strip() in ("1", "true", "yes")
    if asm_token:
        asm = await db.get_assembly_by_public_token(asm_token)
        if not asm:
            return web.json_response({"error": "validation", "message": "Сборка не найдена"}, status=400)
        assembly_id = asm["id"]
    if c_token:
        c = await db.get_contract_by_public_token(c_token)
        if not c:
            return web.json_response({"error": "validation", "message": "Договор не найден"}, status=400)
        contract_id = c["id"]
    # Привязка обязательна, КРОМЕ случая общей публичной формы (general=1)
    if not assembly_id and not contract_id and not is_general:
        return web.json_response({"error": "validation", "message": "Не указан объект (сборка или договор)"}, status=400)
    # Для общей формы требуем хотя бы имя автора — иначе совсем анонимный мусор
    if is_general and not (fields.get("author_name") or "").strip():
        return web.json_response({"error": "validation", "message": "Укажите ваше имя"}, status=400)

    # Создаём замечание
    defect_id = await db.create_defect_report(
        description=description,
        type_=type_,
        assembly_id=assembly_id,
        contract_id=contract_id,
        author_name=fields.get("author_name"),
        author_phone=fields.get("author_phone"),
        location=fields.get("location"),
    )

    # Сохраняем фото на диск + в БД
    defect_dir = os.path.join(DEFECT_PHOTOS_DIR, str(defect_id))
    ...
    for i, f in enumerate(files, start=1):
        ...
        path_rel = os.path.join("defects", str(defect_id), f"p{i}{ext}")
        path_abs = os.path.join(UPLOADS_DIR, path_rel)
        os.makedirs(os.path.dirname(path_abs), exist_ok=True)
        with open(path_abs, "wb") as out:
            out.write(f["data"])
        await db.add_defect_photo(defect_id, path_rel, f["size"], f["content_type"])

    # Журнал
    try:
        await db.log_action(None, "create_defect", "defect", defect_id, description[:200])
    except Exception:
        pass

    return web.json_response({"id": defect_id, "ok": True}, status=201)


async def get_defect_photo_endpoint(request: web.Request) -> web.Response:
    """ПУБЛИЧНАЯ выдача фото замечания по ID. Не требует авторизации.

    Защита — фото доступно только если знать его ID (плюс ссылки приходят
    только тем, у кого есть доступ к замечанию).
    """
    try:
        photo_id = int(request.match_info.get("photo_id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    p = await db._fetch_one("SELECT * FROM defect_report_photos WHERE id = ?", (photo_id,))
    if not p:
        return web.json_response({"error": "not_found"}, status=404)
    path_abs = os.path.join(UPLOADS_DIR, p["file_path"])
    if not os.path.exists(path_abs):
        return web.json_response({"error": "file_missing"}, status=404)
    return web.FileResponse(
        path_abs,
        headers={
            "Content-Type": p.get("content_type") or "application/octet-stream",
...
@require_auth
async def get_defects_list_endpoint(request: web.Request) -> web.Response:
    """Список замечаний с фильтрами."""
    q = request.query
    rows = await db.get_defect_reports(
        status=q.get("status"),
        type_=q.get("type"),
        assembly_id=int(q["assembly_id"]) if q.get("assembly_id", "").isdigit() else None,
        contract_id=int(q["contract_id"]) if q.get("contract_id", "").isdigit() else None,
    )
    return web.json_response({
        "defects": [_serialize_defect(r) for r in rows],
    })


@require_auth
async def get_defect_endpoint(request: web.Request) -> web.Response:
    """Детали замечания."""
    try:
        did = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    d = await db.get_defect_by_id(did)
    if not d:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response(_serialize_defect(d))


@require_defects_manage
async def update_defect_endpoint(request: web.Request) -> web.Response:
    """Смена статуса замечания, ответственного, добавление комментария-резолюции."""
    chat_id = request.get("chat_id")
    try:
        did = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
        return web.json_response({"error": "bad_id"}, status=400)
    existing = await db.get_defect_by_id(did)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        data = await request.json()
    ...
    new_status = data.get("status")
    if new_status and new_status not in db.DEFECT_STATUS_LABELS:
        return web.json_response({"error": "validation", "message": "Некорректный статус"}, status=400)

    await db.update_defect_report(
        did,
        status=new_status,
        resolution_note=data.get("resolution_note"),
        assignee_id=int(data["assignee_id"]) if data.get("assignee_id") not in (None, "", 0) else (None if "assignee_id" in data else None),
        updated_by_chat_id=chat_id,
    )
    try:
        await db.log_action(chat_id, "update_defect", "defect", did, f"status={new_status}")
    except Exception:
        pass
    updated = await db.get_defect_by_id(did)
    return web.json_response(_serialize_defect(updated))


@require_defects_manage
async def delete_defect_endpoint(request: web.Request) -> web.Response:
    chat_id = request.get("chat_id")
    try:
        did = int(request.match_info.get("id", "0"))
    except (TypeError, ValueError):
    ...
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    await db.set_defect_active(did, False)
    try:
        await db.log_action(chat_id, "delete_defect", "defect", did, "")
    except Exception:
        pass
    return web.json_response({"deleted": True})
...
@require_auth
async def get_defects_summary_endpoint(request: web.Request) -> web.Response:
    """Сводка по статусам — для виджета на главной."""
    s = await db.get_defects_summary()
    return web.json_response(s)


@require_auth
async def create_defect_internal_endpoint(request: web.Request) -> web.Response:
    """ЗАЩИЩЁННЫЙ эндпоинт создания замечания (для сотрудников из CRM).

    В отличие от /api/public/defects:
    - Требует авторизации
    - Привязка к сборке/договору ОПЦИОНАЛЬНА — можно создавать внутренние замечания «без привязки»
    - Имя автора берётся из профиля сотрудника, если не указано в форме

    Принимает multipart/form-data с теми же полями что и публичный эндпоинт.
    """
    chat_id = request["chat_id"]
    try:
        reader = await request.multipart()
    ...
    # Привязка к сборке/договору — опциональна
    assembly_id = None
    contract_id = None
    asm_token = (fields.get("assembly_token") or "").strip()
    c_token   = (fields.get("contract_token") or "").strip()
    ...
    if c_token:
        ...
        if not c:
            return web.json_response({"error": "validation", "message": "Договор не найден"}, status=400)
        contract_id = c["id"]
    # Внутреннее замечание — assembly_id и contract_id могут быть None

    # Имя автора — из формы или из профиля
    author_name = (fields.get("author_name") or "").strip()
    if not author_name:
        try:
            emp = await db.get_employee_by_telegram_id(chat_id)
            if emp:
                author_name = emp.get("short_name") or emp.get("full_name") or ""
        except Exception:
            pass
    ...
    try:
        await db.log_action(chat_id, "create_defect", "defect", defect_id, description[:200])
    except Exception:
        pass
...
@require_auth
async def scan_shipment_endpoint(request: web.Request) -> web.Response:
    """Главный endpoint сканирования при отгрузке.

    Body: { "qr_token": "...", "contract_id": int (optional) }

    Возвращает:
      {
        "ok": bool,
        "reason": "ok" | "unknown" | "no_contract" | "wrong_contract" | "already_shipped",
        "item": {"type": "assembly"|"box", "id": int, "name": str, "qty": int},
    ...
    try:
        ...
        return web.json_response({"error": "bad_request", "message": "Тело должно быть JSON"}, status=400)

    qr_token = (data or {}).get("qr_token") or ""
    if not isinstance(qr_token, str) or not qr_token.strip():
        return web.json_response({"error": "bad_request", "message": "qr_token обязателен"}, status=400)
    qr_token = qr_token.strip()

    contract_id_raw = (data or {}).get("contract_id")
    contract_id = None
    if contract_id_raw is not None:
        try:
            contract_id = int(contract_id_raw)
        except (TypeError, ValueError):
            return web.json_response({"error": "bad_request", "message": "contract_id должен быть числом"}, status=400)

    # Если в QR пришёл URL вида https://.../a/TOKEN или /c/TOKEN — извлекаем токен
    # (на случай если фронт передал полную ссылку с наклейки)
    import re
    m = re.search(r'/[ac]/([A-Za-z0-9_\-]+)', qr_token)
    if m:
        qr_token = m.group(1)

    result = await db.scan_shipment(qr_token, contract_id=contract_id, shipped_by=chat_id)

    # Логируем результаты (для аудита)
    try:
        if result.get("ok"):
            item = result.get("item") or {}
            await db.log_action(
                chat_id, "shipment_scan_ok", item.get("type") or "?", item.get("id"),
                f"contract={result.get('contract_id')}, token={qr_token[:8]}",
            )
    except Exception:
        pass
...
@require_auth
async def get_contract_shipment_status_endpoint(request: web.Request) -> web.Response:
    """Возвращает прогресс и список позиций для отгрузки по договору."""
    try:
        contract_id = int(request.match_info["id"])
    ...
    status = await db.get_contract_shipment_status(contract_id)
    return web.json_response(status)


@require_auth
async def undo_shipment_endpoint(request: web.Request) -> web.Response:
    """Удаляет запись об отгрузке (откат ошибочного скана)."""
    chat_id = request["chat_id"]
    try:
    ...
    ok = await db.undo_shipment(shipment_id)
    if not ok:
        return web.json_response({"error": "not_found"}, status=404)

    try:
        await db.log_action(chat_id, "shipment_undo", "shipment", shipment_id, "")
    except Exception:
        pass
...
@require_auth
async def create_box_endpoint(request: web.Request) -> web.Response:
    """Создаёт коробку для договора.

    Body: { "name": str (optional), "description": str (optional) }
    """
    chat_id = request["chat_id"]
    try:
        contract_id = int(request.match_info["id"])
    ...
    contract = await db.get_contract_by_id(contract_id)
    if not contract:
        return web.json_response({"error": "not_found", "message": "Договор не найден"}, status=404)
    ...
    # ЭТАП 26: если имя не задано — генерируем «Коробка #N» по порядку в рамках договора
    # Учитываем удалённые коробки тоже, чтобы номера не повторялись.
    name_clean = str(name).strip()
    if not name_clean:
        seq = await db.count_boxes_in_contract(contract_id, include_deleted=True)
        name_clean = f"Коробка #{seq + 1}"

    box = await db.create_box(
        contract_id=contract_id,
        name=name_clean[:200],
    ...
    try:
        await db.log_action(chat_id, "create_box", "box", box["id"], f"contract={contract_id}")
    except Exception:
        pass

    return web.json_response(box, status=201)


@require_auth
async def list_contract_boxes_endpoint(request: web.Request) -> web.Response:
    """Список коробок договора."""
    try:
        contract_id = int(request.match_info["id"])
    except (KeyError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)
    ...
    boxes = await db.list_boxes_by_contract(contract_id)
    return web.json_response({"boxes": boxes})


@require_auth
async def delete_box_endpoint(request: web.Request) -> web.Response:
    """Soft-delete коробки. Запрещено если коробка уже отгружена."""
    chat_id = request["chat_id"]
    try:
        box_id = int(request.match_info["id"])
    except (KeyError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)

    box = await db.get_box_by_id(box_id)
    if not box:
        return web.json_response({"error": "not_found"}, status=404)

    ok = await db.delete_box(box_id)
    if not ok:
        return web.json_response(
            {"error": "conflict", "message": "Нельзя удалить уже отгруженную коробку"},
            status=409,
        )

    try:
        await db.log_action(chat_id, "delete_box", "box", box_id, "")
    except Exception:
        pass
...
@require_auth
async def update_box_endpoint(request: web.Request) -> web.Response:
    """Переименование коробки.
    ...
    name_arg = data.get("name") if isinstance(data, dict) else None
    desc_arg = data.get("description") if isinstance(data, dict) else None

    if name_arg is not None:
        name_clean = str(name_arg).strip()
        if not name_clean:
            return web.json_response(
                {"error": "validation", "message": "Имя коробки не может быть пустым"},
                status=400,
            )
        name_arg = name_clean

    await db.update_box(box_id, name=name_arg, description=desc_arg)
    try:
        await db.log_action(chat_id, "update_box", "box", box_id, name_arg or "")
    except Exception:
        pass
    updated = await db.get_box_by_id(box_id)
    return web.json_response(updated)


# ============================================================
# ============ ЭТАП 30.1: СОДЕРЖИМОЕ КОРОБОК ==================
# ============================================================


@require_auth
async def list_box_items_endpoint(request: web.Request) -> web.Response:
    """Список позиций коробки."""
    try:
        box_id = int(request.match_info["id"])
    except (KeyError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)
    ...
    if not box:
        return web.json_response({"error": "not_found"}, status=404)
    items = await db.list_box_items(box_id)
    return web.json_response({"items": items})


@require_auth
async def create_box_item_endpoint(request: web.Request) -> web.Response:
    """Добавляет позицию в коробку.

    Body: { name, qty, unit, source_type, source_id, comment }
    """
    chat_id = request["chat_id"]
    try:
        box_id = int(request.match_info["id"])
    ...
    if not name:
        return web.json_response(
            {"error": "validation", "message": "Название позиции не может быть пустым"},
            status=400,
        )
    source_type = data.get("source_type") or "manual"
    if source_type not in db.BOX_ITEM_SOURCE_TYPES:
        source_type = "manual"
    ...
    try:
        ...
        return web.json_response({"error": "validation", "message": str(e)}, status=400)
    try:
        await db.log_action(chat_id, "create_box_item", "box", box_id, name[:100])
    except Exception:
        pass
    return web.json_response(item)
...
@require_auth
async def update_box_item_endpoint(request: web.Request) -> web.Response:
    """Обновляет позицию коробки (name/qty/unit/comment)."""
    chat_id = request["chat_id"]
    try:
    ...
    if not ok:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        await db.log_action(chat_id, "update_box_item", "box_item", item_id, "")
    except Exception:
        pass
    return web.json_response({"ok": True})
...
@require_auth
async def delete_box_item_endpoint(request: web.Request) -> web.Response:
    """Удаляет позицию из коробки."""
    chat_id = request["chat_id"]
    try:
        item_id = int(request.match_info["id"])
    except (KeyError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)
    ok = await db.delete_box_item(item_id)
    if not ok:
        return web.json_response({"error": "not_found"}, status=404)
    try:
        await db.log_action(chat_id, "delete_box_item", "box_item", item_id, "")
    except Exception:
        pass
    return web.json_response({"ok": True})
...
@require_auth
async def get_box_full_endpoint(request: web.Request) -> web.Response:
    """Полные данные коробки для экрана/упаковочного листа: коробка + содержимое + контекст договора."""
    try:
        box_id = int(request.match_info["id"])
    except (KeyError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)
    box = await db.get_box_full(box_id)
    if not box:
        return web.json_response({"error": "not_found"}, status=404)
    return web.json_response(box)
...
@require_auth
async def create_external_shipment_endpoint(request: web.Request) -> web.Response:
    """Создаёт отгрузку без договора.

    Body: {
      recipient_type: 'inn' | 'individual',
      recipient_name: str,
      recipient_inn?: str (если inn),
      recipient_comment: str (обязательно),
    ...
    rec_type = (data.get("recipient_type") or "").strip()
    if rec_type not in db.EXTERNAL_RECIPIENT_TYPES:
        return web.json_response(
            {"error": "validation", "message": "Выберите тип получателя: ИНН или физлицо"},
            status=400,
        )
    rec_name = (data.get("recipient_name") or "").strip()
    if not rec_name:
        return web.json_response(
            {"error": "validation", "message": "Укажите получателя"},
            status=400,
        )
    rec_comment = (data.get("recipient_comment") or "").strip()
    if not rec_comment:
        return web.json_response(
            {"error": "validation", "message": "Комментарий обязателен"},
            status=400,
        )
    rec_inn = (data.get("recipient_inn") or "").strip()
    if rec_type == "inn":
        if not rec_inn.isdigit() or len(rec_inn) not in (10, 12):
            return web.json_response(
                {"error": "validation", "message": "ИНН должен содержать 10 или 12 цифр"},
                status=400,
            )
    items = data.get("items") or []
    if not isinstance(items, list) or not items:
        return web.json_response(
            {"error": "validation", "message": "Нет позиций для отгрузки"},
            status=400,
        )

    result = await db.create_external_shipment(
        items=items,
    ...
    try:
        await db.log_action(chat_id, "external_shipment", "shipment", 0,
                            f"{rec_name} · {result.get('created', 0)} поз.")
    except Exception:
        pass
    status = 200 if result.get("ok") else 207  # 207: частичный успех или ошибки
    return web.json_response(result, status=status)


@require_auth
async def list_external_shipments_endpoint(request: web.Request) -> web.Response:
    """Список внешних отгрузок (для журнала движений)."""
    try:
        limit = int(request.query.get("limit", "100"))
    except (TypeError, ValueError):
        limit = 100
    items = await db.list_external_shipments(limit=limit)
    return web.json_response({"items": items})


async def get_public_box(request: web.Request) -> web.Response:
    """Публичная карточка коробки по QR-токену (без авторизации).

    Показывается когда внешний пользователь сканирует QR коробки обычной камерой.
    Минимум информации: что это коробка, к какому договору и контрагенту.
    """
    token = (request.match_info.get("token") or "").strip()
    if not token:
        return web.json_response({"error": "bad_token"}, status=400)
    box = await db.get_box_by_token(token)
    if not box:
        return web.json_response({"error": "not_found"}, status=404)

    # Контрагент + номер договора уже подтянуты в get_box_by_token (LEFT JOIN)
    return web.json_response({
        "id":              box["id"],
        "name":            box.get("name") or f"Коробка #{box['id']}",
        "description":     box.get("description") or "",
        "qr_token":        box.get("qr_token") or token,
        "contract_id":     box.get("contract_id"),
        "contract_number": box.get("contract_number") or "",
        "contractor_name": box.get("contractor_name") or "",
        "created_at":      box.get("created_at"),
    })


@require_auth
async def qr_lookup_endpoint(request: web.Request) -> web.Response:
    """Универсальный resolver: по QR-токену вернуть тип объекта + контекст.

    ЭТАП 26.3: используется главным сканером для авто-определения куда вести
    пользователя — на карточку сборки/коробки или в режим отгрузки договора.

    Принимает токен в path (`/api/qr/lookup/{token}`) или query (`?token=...`)
    или полную URL (`https://.../a/TOKEN`) — последний случай парсится regex'ом.

    Возвращает:
      {
        "type": "assembly" | "box" | "contract",
        "id": int,
        "name": str,
        "contract_id": int | None,
        "contract_number": str | None,
        "contractor_name": str | None,
        "shipment_progress": {"total": N, "shipped": K} | null,
        "is_shipped": bool   # для assembly/box: уже отгружено?
      }
    либо { "error": "not_found" } / 404.
    """
    token = request.match_info.get("token") or request.query.get("token") or ""
    token = (token or "").strip()
    # Если пришёл URL — извлекаем токен
    import re
    m = re.search(r'/[abc]/([A-Za-z0-9_\-]+)', token)
    if m:
        token = m.group(1)
    if not token:
        return web.json_response({"error": "bad_token"}, status=400)

    # 1. Сначала смотрим — это договор?
    c = await db.get_contract_by_public_token(token)
    if c:
        progress = await db.get_contract_shipment_progress(c["id"])
        return web.json_response({
            "type":              "contract",
            "id":                c["id"],
            "name":              f"Договор {c.get('number') or '#' + str(c['id'])}",
            "contract_id":       c["id"],
            "contract_number":   c.get("number") or "",
            "contractor_name":   c.get("contractor_name") or "",
            "shipment_progress": progress,
            "is_shipped":        False,
        })

    # 2. Иначе — сборка или коробка через универсальный resolve_qr_token
    obj = await db.resolve_qr_token(token)
    if not obj:
        return web.json_response({"error": "not_found"}, status=404)
    ...
    if obj_type == "assembly":
        existing = await db._fetch_one(
            "SELECT id FROM shipments WHERE assembly_id = ?", (obj_id,),
        )
        is_shipped = bool(existing)
    elif obj_type == "box":
        existing = await db._fetch_one(
            "SELECT id FROM shipments WHERE box_id = ?", (obj_id,),
        )
        is_shipped = bool(existing)
    ...
    progress = None
    if contract_id:
        c2 = await db._fetch_one(
            """SELECT c.number, ctr.name AS contractor_name
               FROM contracts c
               LEFT JOIN contractors ctr ON ctr.id = c.contractor_id
               WHERE c.id = ?""",
            (contract_id,),
        ...
        if c2:
            contract_number = c2.get("number") or ""
            contractor_name = c2.get("contractor_name") or ""
        progress = await db.get_contract_shipment_progress(contract_id)
...
def _parse_item_payload(data: dict) -> dict:
    """Нормализует body для create/update позиции."""
    out = {}
    if "name" in data:
        ...
        if v:
            out["name"] = v[:300]
    if "description" in data:
        out["description"] = (data.get("description") or "").strip()[:1000]
    if "unit" in data:
        out["unit"] = (data.get("unit") or "шт.").strip()[:30] or "шт."
    if "qty" in data:
        try:
            out["qty"] = float(data["qty"])
        except (TypeError, ValueError):
            out["qty"] = 1
    if "price" in data:
        try:
    ...
    if "position_no" in data:
        try:
            out["position_no"] = int(data["position_no"])
        except (TypeError, ValueError):
            pass
    if "model_id" in data:
        try:
            mid = data.get("model_id")
            out["model_id"] = int(mid) if mid else None
        except (TypeError, ValueError):
            out["model_id"] = None
    return out
...
@require_auth
async def list_contract_items_endpoint(request: web.Request) -> web.Response:
    """GET /api/contracts/{id}/items — список позиций + итоговая сумма."""
    try:
        contract_id = int(request.match_info["id"])
    except (KeyError, ValueError):
        return web.json_response({"error": "bad_request"}, status=400)
    ...
    items = await db.list_contract_items(contract_id)
    total = await db.get_contract_items_total(contract_id)
    return web.json_response({"items": items, "total": total, "count": len(items)})


@require_auth
async def create_contract_item_endpoint(request: web.Request) -> web.Response:
    """POST /api/contracts/{id}/items — создать позицию.

    Body: { name (required), description?, qty?, unit?, price?, position_no? }
    """
    chat_id = request["chat_id"]
    try:
        contract_id = int(request.match_info["id"])
    ...
    parsed = _parse_item_payload(data or {})
    # Имя обязательно, ЕСЛИ не указан model_id (тогда имя возьмётся из модели)
    if not parsed.get("name") and not parsed.get("model_id"):
        return web.json_response({"error": "bad_request", "message": "Укажите наименование или выберите модель"}, status=400)

    item = await db.create_contract_item(
        contract_id=contract_id,
        ...
        position_no=parsed.get("position_no"),
        model_id=parsed.get("model_id"),
    )

    try:
        await db.log_action(chat_id, "create_contract_item", "contract_item",
                            item["id"], f"contract={contract_id}, name={item['name'][:60]}")
    except Exception:
        pass
...
@require_auth
async def update_contract_item_endpoint(request: web.Request) -> web.Response:
    """PATCH /api/contracts/items/{id} — редактирование позиции."""
    chat_id = request["chat_id"]
    try:
        item_id = int(request.match_info["id"])
    except (KeyError, ValueError):
    ...
    existing = await db.get_contract_item(item_id)
    if not existing:
        return web.json_response({"error": "not_found"}, status=404)
    ...
    parsed = _parse_item_payload(data or {})
    # name явно проверяем: пустое — отказ
    if "name" in (data or {}) and not parsed.get("name"):
        return web.json_response({"error": "bad_request", "message": "Наименование не может быть пустым"}, status=400)

    item = await db.update_contract_item(
        item_id=item_id,
        name=parsed.get("name"),
        description=parsed.get("description"),
        qty=parsed.get("qty"),
    ...
    try:
        await db.log_action(chat_id, "update_contract_item", "contract_item", item_id, "")
    except Exception:
        pass
...
@require_auth
async def delete_contract_item_endpoint(request: web.Request) -> web.Response:
    """DELETE /api/contracts/items/{id} — soft-delete позиции."""
    chat_id = request["chat_id"]
    try:
        item_id = int(request.match_info["id"])
    except (KeyError, ValueError):
    ...
    ok = await db.delete_contract_item(item_id)
    if not ok:
        return web.json_response({"error": "not_found"}, status=404)

    try:
        await db.log_action(chat_id, "delete_contract_item", "contract_item", item_id, "")
    except Exception:
        pass
...
def create_aiohttp_app() -> web.Application:
    ...
    app.router.add_post("/api/auth/password", auth_password)        # ЭТАП 28
    ...
    app.router.add_post("/api/employees/{id}/password", set_employee_password_via_api)   # ЭТАП 28

    # ====== v2.8.2: Справочник должностей ======
    app.router.add_get("/api/positions", get_positions)
    app.router.add_post("/api/positions", create_position_via_api)
    app.router.add_patch("/api/positions/{id}", update_position_via_api)
    app.router.add_delete("/api/positions/{id}", delete_position_via_api)

    # ====== ЭТАП 29: Уровни доступа ======
    app.router.add_get("/api/access-levels", get_access_levels)
    app.router.add_get("/api/access-levels/permissions", get_permissions_registry)
    app.router.add_post("/api/access-levels", create_access_level_via_api)
    app.router.add_patch("/api/access-levels/{id}", update_access_level_via_api)
    app.router.add_delete("/api/access-levels/{id}", delete_access_level_via_api)
    ...
    # ====== ЭТАП 16В-2: Задачи по конкретному договору ======
    app.router.add_get("/api/contracts/{id}/tasks", get_contract_tasks_endpoint)
    ...
    # ====== ЭТАП 18: Склад ======
    app.router.add_get("/api/warehouse/stock", get_warehouse_stock_endpoint)
    app.router.add_get("/api/warehouse/stock/summary", get_warehouse_stock_summary_endpoint)
    app.router.add_get("/api/warehouse/movements", get_warehouse_movements_endpoint)
    app.router.add_post("/api/warehouse/movements", create_warehouse_movement_endpoint)
    app.router.add_get("/api/assemblies/{id}/stock", get_assembly_stock_endpoint)
    app.router.add_patch("/api/assemblies/{id}/status", update_assembly_status_endpoint)
    app.router.add_post("/api/contracts/{id}/ship", ship_contract_endpoint)

    # ====== ЭТАП 19: Снабжение ======
    # Поставщики
    app.router.add_get("/api/suppliers", get_suppliers_endpoint)
    app.router.add_post("/api/suppliers", create_supplier_endpoint)
    app.router.add_get("/api/suppliers/{id}", get_supplier_endpoint)
    app.router.add_patch("/api/suppliers/{id}", update_supplier_endpoint)
    app.router.add_delete("/api/suppliers/{id}", delete_supplier_endpoint)
    # Каталог
    app.router.add_get("/api/supply-items", get_supply_items_endpoint)
    app.router.add_post("/api/supply-items", create_supply_item_endpoint)
    app.router.add_get("/api/supply-items/{id}", get_supply_item_endpoint)
    app.router.add_patch("/api/supply-items/{id}", update_supply_item_endpoint)
    app.router.add_delete("/api/supply-items/{id}", delete_supply_item_endpoint)
    # Заявки
    app.router.add_get("/api/supply-requests", get_supply_requests_endpoint)
    app.router.add_post("/api/supply-requests", create_supply_request_endpoint)
    app.router.add_get("/api/supply-requests/{id}", get_supply_request_endpoint)
    app.router.add_patch("/api/supply-requests/{id}", update_supply_request_endpoint)
    app.router.add_delete("/api/supply-requests/{id}", delete_supply_request_endpoint)
    # Заказы
    app.router.add_get("/api/supply-orders", get_supply_orders_endpoint)
    app.router.add_post("/api/supply-orders", create_supply_order_endpoint)
    app.router.add_get("/api/supply-orders/{id}", get_supply_order_endpoint)
    app.router.add_patch("/api/supply-orders/{id}", update_supply_order_endpoint)
    app.router.add_post("/api/supply-orders/{id}/items", add_supply_order_item_endpoint)
    app.router.add_delete("/api/supply-orders/items/{id}", remove_supply_order_item_endpoint)
    app.router.add_post("/api/supply-orders/{id}/send", send_supply_order_endpoint)
    # Приёмки
    app.router.add_get("/api/supply-receipts", get_supply_receipts_endpoint)
    app.router.add_post("/api/supply-receipts", create_supply_receipt_endpoint)
    app.router.add_get("/api/supply-receipts/{id}", get_supply_receipt_endpoint)
    # Сводка
    app.router.add_get("/api/supply/summary", get_supply_summary_endpoint)

    # ====== ЭТАП 20: Кадры — отпуска ======
    app.router.add_get("/api/vacations",           get_vacations_endpoint)
    app.router.add_post("/api/vacations",          create_vacation_endpoint)
    app.router.add_patch("/api/vacations/{id}",    update_vacation_endpoint)
    app.router.add_delete("/api/vacations/{id}",   delete_vacation_endpoint)
    app.router.add_get("/api/vacations/current",   get_current_vacations_endpoint)

    # ====== ЭТАП 21: Публичные страницы (QR) ======
    # БЕЗ декоратора @require_auth — доступны без логина
    app.router.add_get("/api/public/assembly/{token}", get_public_assembly_endpoint)
    app.router.add_get("/api/public/contract/{token}", get_public_contract_endpoint)
    # Получить токен (для сотрудников чтобы построить QR в карточке)
    app.router.add_get("/api/assemblies/{id}/public-token", get_assembly_token_endpoint)
    app.router.add_get("/api/contracts/{id}/public-token", get_contract_token_endpoint)

    # ====== ЭТАП 22: Доработки — замечания ======
    # Публичные эндпоинты (без авторизации)
    app.router.add_post("/api/public/defects", create_defect_report_endpoint)
    app.router.add_post("/api/defects",           create_defect_internal_endpoint)
    app.router.add_get("/api/defects/photos/{photo_id}", get_defect_photo_endpoint)
    # Для сотрудников
    app.router.add_get("/api/defects",           get_defects_list_endpoint)
    app.router.add_get("/api/defects/summary",   get_defects_summary_endpoint)
    app.router.add_get("/api/defects/{id}",      get_defect_endpoint)
    app.router.add_patch("/api/defects/{id}",    update_defect_endpoint)
    app.router.add_delete("/api/defects/{id}",   delete_defect_endpoint)

    # ============ ЭТАП 26: ОТГРУЗКИ ПО QR ============
    app.router.add_post("/api/shipments/scan",                       scan_shipment_endpoint)
    app.router.add_get( "/api/contracts/{id}/shipment-status",       get_contract_shipment_status_endpoint)
    app.router.add_delete("/api/shipments/{id}",                     undo_shipment_endpoint)
    # Коробки
    app.router.add_post("/api/contracts/{id}/boxes",                 create_box_endpoint)
    app.router.add_get( "/api/contracts/{id}/boxes",                 list_contract_boxes_endpoint)
    app.router.add_patch("/api/boxes/{id}",                          update_box_endpoint)
    app.router.add_delete("/api/boxes/{id}",                         delete_box_endpoint)
    # ЭТАП 26.3: универсальный QR-resolver (для главного сканера)
    app.router.add_get("/api/qr/lookup/{token}",                     qr_lookup_endpoint)
    app.router.add_get("/api/qr/lookup",                             qr_lookup_endpoint)
    # ЭТАП 26: публичная страница коробки (без авторизации)
    app.router.add_get("/api/public/box/{token}",                    get_public_box)

    # ============ ЭТАП 30.1: СОДЕРЖИМОЕ КОРОБОК ============
    app.router.add_get(   "/api/boxes/{id}/full",                    get_box_full_endpoint)
    app.router.add_get(   "/api/boxes/{id}/items",                   list_box_items_endpoint)
    app.router.add_post(  "/api/boxes/{id}/items",                   create_box_item_endpoint)
    app.router.add_patch( "/api/box-items/{id}",                     update_box_item_endpoint)
    app.router.add_delete("/api/box-items/{id}",                     delete_box_item_endpoint)
    # ============ ЭТАП 30.2: ВНЕШНИЕ ОТГРУЗКИ (вне договора) ============
    app.router.add_post(  "/api/shipments/external",                 create_external_shipment_endpoint)
    app.router.add_get(   "/api/shipments/external",                 list_external_shipments_endpoint)

    # ============ ЭТАП 27: СПЕЦИФИКАЦИЯ ДОГОВОРА ============
    app.router.add_get(   "/api/contracts/{id}/items",  list_contract_items_endpoint)
    app.router.add_post(  "/api/contracts/{id}/items",  create_contract_item_endpoint)
    app.router.add_patch( "/api/contracts/items/{id}",  update_contract_item_endpoint)
    app.router.add_delete("/api/contracts/items/{id}",  delete_contract_item_endpoint)
