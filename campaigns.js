...
var CAMPAIGN_SIGNATURE = 'С уважением,\nДмитрий\nАтомус Групп\nТелефон менеджера: +7 992 507-48-16';
var CAMPAIGN_TEMPLATES = {
  new: {"label": "Камера созревания — получить расчёт", "subject": "Камера созревания сыра: какое оборудование нужно и сколько оно будет стоить?", "body": "Добрый день, {{contact}}!\n\nПланируете камеру созревания сыра или хотите улучшить работу существующей? Предлагаем начать с расчёта оборудования под ваш сыр, помещение и объём производства.\n\nМы в «Атомус Групп» занимаемся климатическим оборудованием для камер созревания. Подготовим предложение с составом оборудования и стоимостью — чтобы вы могли оценить бюджет и сравнить варианты.\n\nДля первого шага достаточно ответить на это письмо:\n• какой сыр производите;\n• размеры камеры или её план;\n• новая камера нужна или модернизация существующей.\n\nЕсли знаете нужные температуру и влажность — укажите их. Если готового ТЗ нет, уточним исходные данные вместе. Уже есть ТЗ или перечень оборудования? Присылайте — подготовим свой вариант для сравнения.\n\nКакая камера нужна вам? Отправьте размеры и пару слов о задаче — начнём с этого."},
  upgrade: { label: 'Действующая камера — модернизация', subject: 'Что хотели бы улучшить в вашей камере созревания?', body: 'Добрый день, {{contact}}!\n\nВопрос по вашей камере созревания: есть ли что-то в работе климата, что вы хотели бы улучшить?\n\nНапример, стабильность температуры и влажности, распределение воздуха или удобство управления.\n\nМы в «Атомус Групп» можем рассмотреть вашу задачу и подготовить вариант подбора оборудования для модернизации.\n\nОтветьте буквально в двух словах, что сейчас не устраивает. Если удобно — приложите фото камеры и установленного оборудования. Начнём с этого, затем уточним данные для расчёта.' },
  alternative: { label: 'Выбор подрядчика — расчёт по ТЗ', subject: 'Ещё один вариант расчёта оборудования для вашей камеры', body: 'Добрый день, {{contact}}!\n\nЕсли вы сейчас выбираете оборудование для камеры созревания, предлагаем подготовить расчёт от «Атомус Групп».\n\nМожно прислать уже готовое ТЗ, план помещения или перечень оборудования — повторно описывать всю задачу не потребуется.\n\nПодготовим своё предложение с составом оборудования и стоимостью, чтобы у вас был ещё один вариант для сравнения.\n\nЕсли исходных данных пока немного, начнём с размеров камеры, вида сыра и планируемой загрузки.' },
...
function campaignsProgress(c) {
  ...
  const quota = c.quota;
  const waiting = c.status === 'running' && queued > 0 && quota && quota.next_at * 1000 > Date.now();
  const quotaText = quota ? 'За последние 24 часа: ' + quota.used + ' из ' + quota.limit + ' писем. Доступно: ' + quota.remaining + '.' : '';
  const title = waiting && !scheduled ? 'Ожидаем доступный лимит отправки' : scheduled ? 'Отправка запланирована' : ({running:'Идёт отправка',paused:'Отправка на паузе',completed:'Отправка завершена',cancelled:'Рассылка остановлена'}[c.status] || 'Статус рассылки');
  return '<div class="campaign-progress" role="status" aria-live="polite"><div class="campaign-progress-top"><strong>' + title + '</strong><span>' + sent + ' из ' + total + ' передано сервису</span></div><progress max="' + Math.max(1,total) + '" value="' + done + '" aria-label="Обработано писем"></progress><p>В очереди: <b>' + queued + '</b> · Отправляется: <b>' + sending + '</b> · Доставлено: <b>' + delivered + '</b> · Возвраты: <b>' + bounced + '</b> · Ошибки / нужна проверка: <b>' + failed + '</b></p><small>' + (waiting && !scheduled ? 'Автоматическая повторная попытка: ' + new Date(quota.next_at * 1000).toLocaleString('ru-RU') : scheduled ? 'Начало: ' + new Date(c.scheduled_at * 1000).toLocaleString('ru-RU') : c.status === 'running' ? 'Можно закрыть страницу — отправка продолжится на сервере.' : 'Статусы доставки могут поступать позже.') + '</small><small>' + campaignsEscape(quotaText) + '</small><small id="campaign-live-update">Обновляем статус каждые 10 секунд</small></div>';
}
...
function campaignsEditor() {
  ...
  panel.innerHTML = '<div class="campaign-head"><div><h2>Подготовка рассылки</h2><p>Выберите получателей, добавьте материалы и подготовьте письмо.</p></div><button class="btn btn-secondary" onclick="campaignsList()">К рассылкам</button></div>' +
    '<nav class="campaign-steps" aria-label="Шаги подготовки">' + ['Получатели', 'Материалы', 'Письмо', 'Проверка и запуск'].map((v, i) => '<button class="btn btn-secondary" onclick="campaignsStep(' + i + ')"><span class="campaign-step-number">' + (i + 1) + '.</span><span>' + v + '</span></button>').join('') + '</nav>' +
...
function campaignsTemplate(k) {
  if (!CAMPAIGN_TEMPLATES[k] || !confirm('Заменить тему, текст и подпись выбранным шаблоном?')) return;
  document.getElementById('campaign-subject').value = CAMPAIGN_TEMPLATES[k].subject;
  document.getElementById('campaign-body').value = CAMPAIGN_TEMPLATES[k].body;
  document.getElementById('campaign-signature').value = CAMPAIGN_SIGNATURE;
  campaignsSync();
}
...
function campaignsConsentToggle() {
  const panel = document.getElementById('campaign-consent');
  panel.hidden = !panel.hidden;
}
async function campaignsBulkConsent() {
  const basis = document.getElementById('campaign-consent-basis').value.trim();
  const date = document.getElementById('campaign-consent-date').value;
  const ids = _campaigns.draft.recipient_ids.slice();
  if (!ids.length || !basis || !date) throw new Error('Выберите получателей, укажите основание и дату согласия');
  const result = await campaignsRequest('/bulk-consent', 'POST', {recipient_ids: ids, basis: basis, date: date});
  _campaigns.preview = null;
  document.getElementById('campaign-consent-result').textContent = 'Согласие зафиксировано: ' + result.updated + '. Без изменений: ' + result.skipped + '.';
  document.getElementById('campaign-consent').hidden = true;
  await campaignsAudience();
}
